...
static GLuint glmFindOrAddTexture(GLMmodel* model, const char* name)
{
	...
	/* didn't find the name, so print a warning and return the default
	 texture (0). */
	model->numtextures++;
	model->textures = (GLMtexture*) realloc(model->textures,
			sizeof(GLMtexture) * model->numtextures);
	model->textures[model->numtextures - 1].name = strdup(name);
	model->textures[model->numtextures - 1].id = glmLoadTexture(filename,
			GL_TRUE, GL_TRUE, GL_TRUE, GL_TRUE, &width, &height);
	...
	while (fscanf(file, "%s", buf) != EOF)
	{
		...
		case 'i':
			if (strncmp(buf, "illum", 5) != 0)
				__glmFatalError(
						"glmReadMTL: Got \"%s\" instead of \"illum\" in file \"%s\"",
						buf, filename);
...
static GLvoid glmWriteMTL(GLMmodel* model, char* modelpath, char* mtllibname)
{
	...
	filename = (char*) malloc(
			sizeof(char) * (strlen(dir) + strlen(mtllibname) + 1));
	strcpy(filename, dir);
	...
	while (fscanf(file, "%s", buf) != EOF)
	{
		...
		case 'm':
			if (strncmp(buf, "mtllib", 6) != 0)
				__glmFatalError("glmReadOBJ: Got \"%s\" instead of \"mtllib\"",
						buf);
			fgets(buf, sizeof(buf), file);
			sscanf(buf, "%s %s", buf, buf);
		...
		case 'u':
			if (strncmp(buf, "usemtl", 6) != 0)
				__glmFatalError("glmReadOBJ: Got \"%s\" instead of \"usemtl\"",
						buf);
			/* eat up rest of line */
			fgets(buf, sizeof(buf), file);
			break;
 ...
 */
GLvoid glmVertexNormals(GLMmodel* model, GLfloat angle, GLboolean keep_existing)
{
	GLMnode* node;
	GLMnode* tail;
	GLMnode** members;
	...
	for (i = 1; i <= model->numvertices; i++)
	{
		...
		while (node)
		{
			...
			if (((int) (T(node->index).findex) != -1)
					&& ((int) (T(members[i]->index).findex) != -1))
			{
				...
				if (dot > cos_angle)
				{
					...
					average[0] += model->facetnorms[3 * T(node->index).findex
							+ 0];
					average[1] += model->facetnorms[3 * T(node->index).findex
							+ 1];
					average[2] += model->facetnorms[3 * T(node->index).findex
							+ 2];
				}
			}
			node = node->next;
		...
		while (node)
		{
				...
				for (j = 0; j < 3; j++)
				{
					...
					if (T(node->index).vindices[j] == i)
					{
						if (T(node->index).nindices[j] > numnormals)
							assert(
									(int)(T(node->index).nindices[j]) == -1 || T(node->index).nindices[j] <= model->numnormals);
						if (!keep_existing
								|| (int) (T(node->index).nindices[j]) == -1)
						{
							if (avg_index == -1)
							{
			...
			else if ((int) (T(node->index).findex) != -1)
			{
				int discard = 1;
				...
				while (model->numnormals < numnormals)
				{
					...
					model->numnormals += 100;
					model->normals = (GLfloat*) realloc(model->normals,
							sizeof(GLfloat) * 3 * (model->numnormals + 1));
				}
				assert(
						(int)(T(node->index).findex) == -1 || T(node->index).findex <= model->numfacetnorms);
				assert(model->numnormals >= numnormals);
		...
		while (group)
		{
			...
			for (i = 0; i < group->numtriangles; i++)
			{
				...
				if (blending == (int) blenditer)
				{
					...
					{
						if (mode & GLM_SMOOTH
								&& ((int) (triangle->nindices[j]) != -1))
						{
							assert(
									triangle->nindices[j] >= 1
											&& triangle->nindices[j]
													<= model->numnormals);
							glNormal3fv(
									&model->normals[3 * triangle->nindices[j]]);
						}
						if (mode & GLM_TEXTURE
								&& ((int) (triangle->tindices[j]) != -1)
								&& (int) map_diffuse != -1)
						{
							//assert(map_diffuse >= 0 && map_diffuse < model->numtextures);		//There was a warning because map_diffuse >= 0 is always true
							assert(map_diffuse < model->numtextures);
							assert(
									triangle->tindices[j] >= 1
											&& triangle->tindices[j]
													<= model->numtexcoords);
							glTexCoord2f(model->texcoords[2 * triangle->tindices[j]] * model->textures[map_diffuse].width,
									(1.0 - model->texcoords[2 * triangle->tindices[j] + 1]) * model->textures[map_diffuse].height);
						}
						assert(
								triangle->vindices[j] >= 1
										&& triangle->vindices[j]
												<= model->numvertices);
						glVertex3fv(
								&model->vertices[3 * triangle->vindices[j]]);
					}
				}

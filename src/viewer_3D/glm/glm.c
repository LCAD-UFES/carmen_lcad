...
glmVertexNormals(GLMmodel* model, GLfloat angle, GLboolean keep_existing)
{
	...
	/* calculate an average normal for this vertex by averaging the
	   facet normal of every triangle this vertex is in */
	    ...
	    if (((int)(T(node->index).findex) != -1) && ((int)(T(members[i]->index).findex) != -1)) {
	...
	avg_index = -1;
		...
		for (j = 0; j<3; j++) {
			...
			assert((int)(T(node->index).nindices[j]) == -1 || T(node->index).nindices[j] <= model->numnormals);
			if (!keep_existing || (int)(T(node->index).nindices[j]) == -1) {
			    if (avg_index == -1) {
		...
		while (model->numnormals < numnormals) {
		    ...
		    model->normals = (GLfloat*)realloc(model->normals, sizeof(GLfloat)* 3 * (model->numnormals+1));
                ...
                assert((int)(T(node->index).findex) == -1 || T(node->index).findex <= model->numfacetnorms);
		assert(model->numnormals >= numnormals);

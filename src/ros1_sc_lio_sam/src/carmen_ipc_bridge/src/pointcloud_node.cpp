...
public:
    ...
    void on_trailer_pose(double beta, bool engaged)
    {
        trailer_beta_.store(beta);
...
{
    if (!g_pc_node || !msg) return;
    // Ângulo de articulação relativo entre o semi-trailer 0 e o caminhão.
    double beta = convert_theta1_to_beta(msg->globalpos.theta, msg->trailer_theta[0]);
    g_pc_node->on_trailer_pose(beta, msg->semi_trailer_engaged != 0);
}
...
void PointcloudNode::load_carmen_lidar_config(int argc, char **argv)
{
    ...
    // Substitua ou atualize esta seção em pointcloud_node.cpp
...
void PointcloudNode::publish_static_transforms(const ros::Time &stamp)
{
    ...
    tf_map_base.transform.translation.x = -T_robot_lidar.getOrigin().x();
    tf_map_base.transform.translation.y = -T_robot_lidar.getOrigin().y();
    tf_map_base.transform.translation.z = -T_robot_lidar.getOrigin().z();
    tf_map_base.transform.rotation.x    = -T_robot_lidar.getRotation().x();
    tf_map_base.transform.rotation.y    = -T_robot_lidar.getRotation().y();
    tf_map_base.transform.rotation.z    = -T_robot_lidar.getRotation().z();
    tf_map_base.transform.rotation.w    = T_robot_lidar.getRotation().w();
    ...
    tf_base_gnd_map.transform.translation.x = -T_ground_robot_.getOrigin().x();
    tf_base_gnd_map.transform.translation.y = -T_ground_robot_.getOrigin().y();
    tf_base_gnd_map.transform.translation.z = -T_ground_robot_.getOrigin().z();
    tf_base_gnd_map.transform.rotation.x    = -T_ground_robot_.getRotation().x();
    tf_base_gnd_map.transform.rotation.y    = -T_ground_robot_.getRotation().y();
    tf_base_gnd_map.transform.rotation.z    = -T_ground_robot_.getRotation().z();
    tf_base_gnd_map.transform.rotation.w    = T_ground_robot_.getRotation().w();

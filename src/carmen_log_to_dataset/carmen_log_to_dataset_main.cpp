...
image_handler(carmen_bumblebee_basic_stereoimage_message* image_msg)
{
	...
	// write left image
	_mkdir("/dados/dataset/log_images/images_left/");
	sprintf(image_filename, "/dados/dataset/log_images/images_left/%08d.png", image_number);
	cv::imwrite(image_filename, *left_image);

	// write right image
	_mkdir("/dados/dataset/log_images/images_right/");
	sprintf(image_filename, "/dados/dataset/log_images/images_right/%08d.png", image_number);
	cv::imwrite(image_filename, *right_image);

	// write timestamps file
	sprintf(timestamps_filename, "/dados/dataset/log_images/timestamps.txt");
...
velodyne_handler(carmen_velodyne_partial_scan_message *velodyne_message)
{
	...
	// write velodyne
	_mkdir("/dados/dataset/log_velodyne/velodyne");
	sprintf(velodyne_filename, "/dados/dataset/log_velodyne/velodyne/%08d.bin", velodyne_number);
	...
	// write timestamps file
	sprintf(timestamps_filename, "/dados/dataset/log_velodyne/timestamps.txt");
...
shutdown_module(int signo)
{
	if (signo == SIGINT)
	{
		...
		printf("Carmen log to dataset: disconnected.\n");
		exit(0);
	}
}
///////////////////////////////////////////////////////////////////////////////////////////////

...
void
remove_clusters_of_static_obstacles_using_detections(int camera_index, int image_index, vector<vector<image_cartesian>> &clustered_points, 
			 ...
			 vector<vector<image_cartesian>> clustered_points)
{
	...
	{
		remove_clusters_of_static_obstacles_using_detections(camera_index, image_index, clustered_points, image_width, image_height);
	}
...
filter_sensor_data_using_images(sensor_parameters_t *sensor_params, sensor_data_t *sensor_data)
{
	...
	for (int camera = 1; camera <= MAX_CAMERA_INDEX; camera++)
	{
		...
		clusters = process_image(sensor_params, sensor_data, camera, nearest_index, clustered_points);
...
usage()
{
	const char *msg = (const char *)
		...
		"    -neural_network <n>                    : neural_network option (yolo)\n"
		"\n";

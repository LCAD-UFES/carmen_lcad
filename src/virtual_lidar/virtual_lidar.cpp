...
// 	points = compute_obstacle_points(&sensor_params[VELODYNE], sensor_data);
// 	points_sick = compute_obstacle_points(&sensor_params[LASER_LDMRS], sensor_data);

// 	clustered_points = dbscan_compute_clusters(0.5, 1, points);
// 	clustered_points_sick = dbscan_compute_clusters(0.5, 2, points_sick);
...
// 		// 	continue;
// 		sprintf(arr,"%lf", camera_data[camera].timestamp[nearest_index]);
// 		cout << arr;


// 		clusters = process_image(sensor_params, sensor_data, camera, nearest_index, clustered_points, clustered_points_sick);

		...
		double initial_time, double final_time, PsoData *pso_data)
{
	...
	//alteracoes feitas por cavalcante
	if (only_odometry == 1)
		first_gps_timestamp = 1;
...
//	FILE *yaw_file = NULL;
	...
	while(!feof(f) && num_lines < max_log_lines)
	{
		...
		else if (!strcmp(tag, "ROBOTVELOCITY_ACK") && (first_gps_timestamp != 0.0))
		{
			...
			//alteracoes feitas por cavalcante
			if (only_odometry == 1 && first_gps_timestamp == 1)
			{
				first_gps_timestamp = m.timestamp;
			}
			if ((m.timestamp >= (first_gps_timestamp + initial_time)) && (m.timestamp <= (first_gps_timestamp + final_time)))
...
//	exit(0);
	...
	printf("Done.\n");
//alteracoes feitas por cavalcante
	if (!only_odometry && pso_data->gps_data.size() <= 0)
...
print_result(double *particle, FILE *f_report, PsoData *pso_data, int *id_first_pose, int particle_id)
{
	...
	double x;
	double y;
	//alteracoes feitas por cavalcante e marcos 
	if(only_odometry==0)
	{
	...
	{
		double v = pso_data->odometry_data[0].v;
		double phi = pso_data->odometry_data[0].phi;
		double timestamp = pso_data->odometry_data[0].odometry_timestamp;
		ackerman_model(x, y, yaw,v,phi,timestamp , L);
	}
...
size_t
find_nearest_time_to_odometry(PsoData *pso_data, int id_odo, double latency)
{
	size_t i = 0;

	while (pso_data->velodyne_data[i].velodyne_timestamp < (pso_data->odometry_data[id_odo].odometry_timestamp + latency))
		i++;
...
double
save_poses_in_odometry_format(PsoData *pso_data, const string &path)
{
	if (path.size() <= 0)
		return (0.0);
	...
	//double *particle = optimizer.GetBestSolution();
	FILE *fptr = safe_fopen(path.c_str(), "w");
	double particle[4] = {0.0, 0.0, 1.0, 0.0};
	double yaw = 0;
	double x = 0;
	double y = 0;
	...
	for (uint i = 1; i < pso_data->odometry_data.size(); i++)
	{
		double global_x = 0;
		double global_y = 0;
		...
		{
			first_sample = i - 1;
			velodyne_sample = find_nearest_time_to_odometry(pso_data, 0, 0);
		}

		timestamp = pso_data->odometry_data[i - 1].odometry_timestamp;
		while ((timestamp < pso_data->odometry_data[i].odometry_timestamp) && (velodyne_sample < pso_data->velodyne_data.size()))
		{
			double dt = pso_data->velodyne_data[velodyne_sample].velodyne_timestamp - timestamp;

			ackerman_model(x,y,yaw,pso_data->odometry_data[i-1].v, pso_data->odometry_data[i-1].phi,dt,L);
...
initialize_parameters(PsoData &pso_data, CommandLineArguments *args, CarmenParamFile *carmen_ini_params)
{
	...
	//alteracoes feitas por marcos
	only_odometry = args->get<int>("only_odometry");
...
main(int argc, char **argv)
{
	...
	if (only_odometry == 1)
	{
		save_poses_in_odometry_format(&pso_data, args.get<string>("poses_opt"));
		return (0);
	}
	srand(time(NULL));
	srand(rand()); // ??

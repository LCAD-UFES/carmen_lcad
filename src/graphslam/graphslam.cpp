...
generate_poses_in_gps_coordinates(char *filename, char *filename_in_gps_coordinates, int gps_id)
{
	...
	while (!feof(f))
	{
		...
		fprintf(f2, "%lf %lf %lf %lf %lf\n", previous_gps_x, previous_gps_y, vector_size * cos(previous_gps_theta), vector_size * sin(previous_gps_theta), previous_timestamp);
...
plot_graph(int gps_id)
{
	...
	fprintf(gnuplot_pipe, "set size square; set size ratio -1; plot 'tmp/sync.txt' u 4:5 w l t 'gps xyz', 'tmp/poses_opt.txt' u 1:2 w l t 'car', 'tmp/poses_opt_in_gps_coordinates.txt' u 1:2:3:4 with vectors title 'car in gps coords'\n");

...
void
publish_neural_motion_planner_motion_commands(vector<carmen_robot_and_trailers_path_point_t> path, double timestamp)
{
...
build_and_follow_path(double timestamp)
{
	...
	if (GlobalState::goal_pose && (GlobalState::route_planner_state != PLANNING_FROM_POSE_TO_LANE))
	{
		...
		else
		{
			...
			{
				//printf("phi nao excedido e path.size > 0\n");//@CAIO: entrou aqui
				if (GlobalState::eliminate_path_follower) {
					//printf("eliminando path follower\n");//@CAIO: entrou aqui

...
vector< vector<Point> > g_points_to_spline;
...
on_mouse(int event, int x, int y, int flag, void *param)
{
	...
	int i = g_points_to_spline.size() - 1;
...
void
draw_grid(int lines_orientation, int delta_parts)
{
	static int parts = 10 + delta_parts;
	static int dist = 0;

	if (lines_orientation%2 == 0) // horizontal
	{
		dist = image.rows / parts;
		for (int i = 0; i < image.rows; i += dist)
			cv::line(image, Point(0,i), Point(image.cols, i), cv::Scalar(0, 0, 255));
	}
	else
	{
		dist = image.cols / parts;
		for (int i = 0; i < image.cols; i += dist)
			cv::line(image, Point(i,0),Point(i, image.rows), cv::Scalar(0, 0, 255));
	}
}
...
main(int argc, char** argv)
{
	...
	if (input.is_open())
	{
		...
		while (!input.eof())
		{
			int lines_orientation = 0;
			int n_click_s = 0;
			for (int k = 0; k < g_points_to_spline.size(); k++)
				g_points_to_spline[k].clear();
			g_points_to_spline.clear();
			...
			if (!image.empty())
			{
				...
				{																//<D> key 100 Save image to DISCARDED file
					...
					case 115:						// s key 115
						...
						{
							vector<Point> points;
							g_points_to_spline.push_back(points);
							setMouseCallback(window_name, on_mouse, NULL);
						}
						else
						{
							setMouseCallback(window_name, NULL, NULL);
							// make spline and draw it on image
							int l = g_points_to_spline.size() - 1;
							vector<double> X, Y;

							for (int j = 0; j < g_points_to_spline[l].size(); j++)
							{
								Point p = g_points_to_spline[l][j];
								printf("x=%d y=%d\n", p.x, p.y);
								X.push_back(p.x);
								Y.push_back(p.y);
							}
							...
							{
								Point c = Point(k, s(k));
								Point c2 = Point(k+1, s(k+1));
								cv::line(image, c, c2, Scalar(255, 0, 0));
								imshow(window_name, image);
							}
					...
					case 103:                     // g key 103 draw horizontal/vertical lines in image
						draw_grid(lines_orientation++, 0);

...
void ELAS::lmt_classification(const pre_processed * _pre_processed, const feature_maps * _feature_maps, const lane_position * _lane_position, lane_marking_type * out) {
	printf("lmt_classification()\n");

	vector<LMT> LMTs = lmt_detector->executar(_feature_maps->map_inb_ipm, _pre_processed->colorFrame.clone(), _pre_processed->grayFrameRoiIPM, _lane_position->trustworthy_height, _lane_position->lane_base);

/*
 * Module test for the obstacle avoider algorithm
 * Sends a motion command with continuous velocity of 10m/s and steering angle of 0 to the obstacle avoider
 * This module was based on the ford_escape_hybrid_train_base.c
 *
 */
...
build_trajectory_stop_smooth_trajectory()
{
	...
	//rintf("i = %d, NUM_MOTION_COMMANDS_PER_VECTOR = %d\n", i, NUM_MOTION_COMMANDS_PER_VECTOR);
	send_trajectory_to_robot();
}
*/
...
define_messages()
{
	...
	err = IPC_defineMsg(CARMEN_ROBOT_ACKERMAN_MOTION_COMMAND_NAME,
			IPC_VARIABLE_LENGTH, CARMEN_ROBOT_ACKERMAN_MOTION_COMMAND_FMT);

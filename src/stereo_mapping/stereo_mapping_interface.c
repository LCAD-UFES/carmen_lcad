...
char *carmen_stereo_mapping_get_messagename(int camera) {
  ...
  {
  case 1:
    return ((char *) CARMEN_STEREO_MAPPING_MSG_0_NAME);

  case 2:
    return ((char *) CARMEN_STEREO_MAPPING_MSG_1_NAME);
  }
    ...
    int camera)
{
  ...
  carmen_subscribe_message(msg_name,
      (char *) CARMEN_STEREO_MAPPING_FMT,
...
carmen_stereo_mapping_define_messages(int camera)
{
  ...
  err = IPC_defineMsg(stereo_mapping_msg_name, IPC_VARIABLE_LENGTH, (char *) CARMEN_STEREO_MAPPING_FMT);
  carmen_test_ipc_exit(err, (char *) "Could not define", stereo_mapping_msg_name);

...
Neural_map::clear_maps()
{
	for (int y = 0; y < this->size_y; y++)
	{
		...
		{
			this->raw_min_hight_map.map[x][y]= -1.;
			this->raw_max_hight_map.map[x][y]= -1.;
			this->raw_mean_hight_map.map[x][y]= -1.;
			this->raw_square_sum_map.map[x][y]= -1.;
			this->raw_number_of_lasers_map.map[x][y]= -1.;
			this->neural_mapper_occupancy_map.map[x][y]= -1.;
		}
	}
}

...
{
	if ((int) strlen(message) > status_message_max_size)
	{
...
static gint
radio_key_release(GtkWidget *w __attribute__ ((unused)), GdkEventKey *event, gpointer pntr __attribute__ ((unused)))
{

	if ((event->keyval == gdk_keyval_from_name("Enter")) || (event->keyval == gdk_keyval_from_name("Return")))
...
static gint
entry_key_release(GtkWidget *w, GdkEventKey *event, gpointer pntr __attribute__ ((unused)))
{
	GType type;
...
{
	FILE *fin, *fout;
	char rpin[PATH_MAX], rpout[4 * PATH_MAX];
	...
	int count;
	int m, p, i;
	char include_path[PATH_MAX], rp_include_filename[PATH_MAX], buffer[4 * PATH_MAX];
	int num_items;
...
static void
file_dialog_destroy(GtkWidget *w __attribute__ ((unused)), gpointer p __attribute__ ((unused)))
{
	file_dialog = NULL;
}
...
file_dialog_init()
{
	...
	gtk_signal_connect(GTK_OBJECT(ok_button), "clicked", GTK_SIGNAL_FUNC(file_dialog_ok), NULL);
	gtk_signal_connect(GTK_OBJECT(cancel_button), "clicked", GTK_SIGNAL_FUNC(file_dialog_cancel), NULL);
	gtk_signal_connect(GTK_OBJECT(file_dialog), "destroy", GTK_SIGNAL_FUNC(file_dialog_destroy), NULL);
	...
	FALSE, FALSE, 10);
	gtk_box_pack_start(GTK_BOX(GTK_DIALOG(file_dialog)->action_area), cancel_button, FALSE, FALSE, 10);
}
...
static void
file_window_destroy(GtkWidget *w __attribute__ ((unused)), gpointer p __attribute__ ((unused)))
{
	file_window = NULL;
}
...
file_ok()
{
	strncpy(ini_filename, gtk_file_selection_get_filename(GTK_FILE_SELECTION(file_window)), 1023);
	ini_filename[1022] = '\0';
...
file_window_init()
{
	...
	gtk_signal_connect(GTK_OBJECT(GTK_FILE_SELECTION(file_window)->ok_button), "clicked", GTK_SIGNAL_FUNC(file_ok), NULL);
	gtk_signal_connect(GTK_OBJECT(GTK_FILE_SELECTION(file_window)->cancel_button), "clicked", GTK_SIGNAL_FUNC(file_cancel), NULL);
	gtk_signal_connect(GTK_OBJECT(file_window), "destroy", GTK_SIGNAL_FUNC(file_window_destroy), NULL);
}
...
static void
window_destroy(GtkWidget *w __attribute__ ((unused)), gpointer p __attribute__ ((unused)))
{
	gui_shutdown();
	params_shutdown();
	gtk_main_quit();
...
menubar_init(GtkWidget *window)
{
  ...
  menubar = gtk_item_factory_get_widget(item_factory, "<main>");
	...
	{ "Save_ini", GTK_STOCK_SAVE, "_Save ini", "<control>S", NULL, file_window_popup },
	{ "Quit", GTK_STOCK_QUIT, "_Quit", "<control>Q", NULL, G_CALLBACK(window_destroy) },
	...
	gtk_action_group_add_actions(action_group, action_entries, G_N_ELEMENTS(action_entries), window);
...
radio_button_toggled(GtkWidget *radio_button, gpointer data)
{
	...
	carmen_test_alloc(values[m][p]);
	if ((radio_button == radio_buttons[m][p][0]) && carmen_strncasecmp(values[m][p], "on", 2))
		strcpy(values[m][p], "on");
	else if ((radio_button == radio_buttons[m][p][1]) && carmen_strncasecmp(values[m][p], "off", 3))
...
notebook_init()
{
	...
	for (m = 0; m < num_modules; m++)
	{
		...
		for (p = 0; p < num_params[m]; p++)
		{
			...
			if (expert[m][p])
				gtk_table_attach_defaults(GTK_TABLE(expert_tables[m]), labels[m][p], 0, 1, expert_cnt, expert_cnt + 1);
			else
				gtk_table_attach_defaults(GTK_TABLE(basic_tables[m]), labels[m][p], 0, 1, basic_cnt, basic_cnt + 1);
			if (!carmen_strncasecmp(values[m][p], "on", strlen(values[m][p])) || !carmen_strncasecmp(values[m][p], "off", strlen(values[m][p])))
			{
				...
				radio_buttons[m][p][1] = gtk_radio_button_new_with_label(gtk_radio_button_group(GTK_RADIO_BUTTON(radio_buttons[m][p][0])), "off");
				...
				param_id->m = m;
				param_id->p = p;
				gtk_signal_connect(GTK_OBJECT(radio_buttons[m][p][0]), "clicked", GTK_SIGNAL_FUNC(radio_button_toggled), (gpointer) param_id);
				gtk_signal_connect(GTK_OBJECT(radio_buttons[m][p][1]), "clicked", GTK_SIGNAL_FUNC(radio_button_toggled), (gpointer) param_id);
				gtk_signal_connect(GTK_OBJECT(radio_buttons[m][p][0]), "key_release_event", GTK_SIGNAL_FUNC(radio_key_release), NULL);
				gtk_signal_connect(GTK_OBJECT(radio_buttons[m][p][1]), "key_release_event", GTK_SIGNAL_FUNC(radio_key_release), NULL);
			...
			else
			{
				...
				param_id->m = m;
				param_id->p = p;
				gtk_signal_connect(GTK_OBJECT(entries[m][p]), "changed", GTK_SIGNAL_FUNC(entry_changed), (gpointer) param_id);
				gtk_signal_connect(GTK_OBJECT(entries[m][p]), "key_release_event", GTK_SIGNAL_FUNC(entry_key_release), NULL);

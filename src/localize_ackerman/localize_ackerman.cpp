...
read_parameters(int argc, char **argv, carmen_localize_ackerman_param_p param, ProbabilisticMapParams *p_map_params)
{
	...
	param->xy_uncertainty_due_to_grid_resolution = (p_map_params->grid_res / 4.0) * (p_map_params->grid_res / 4.0);
	param->yaw_uncertainty_due_to_grid_resolution = asin((p_map_params->grid_res / 4.0) / max_range) * asin((p_map_params->grid_res / 4.0) / max_range);

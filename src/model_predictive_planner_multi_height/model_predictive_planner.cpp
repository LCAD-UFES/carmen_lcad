...
path_has_collision_or_phi_exceeded(vector<carmen_ackerman_path_point_t> path)
{
	...
	for (unsigned int i = 0; i < path.size(); i += 1)
	{
		...
		if (GlobalState::distance_map != NULL && GlobalState::distance_map_level1 != NULL)
		{
			carmen_obstacle_distance_mapper_map_message *distance_maps[] = {GlobalState::distance_map, GlobalState::distance_map_level1};
			double circle_invasion = sqrt(carmen_obstacle_avoider_compute_car_distance_to_closest_obstacles_multi_height(&localizer,

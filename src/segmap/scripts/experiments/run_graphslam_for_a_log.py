...
def run_odom_calib(carmen_path, log_path, output_dir, config_dir):
	...
	config_path = ""
	cmd = "%s %s %s %s %s %s %s --save %s" % (program, log_path, PARAM_FILE, output_path, report_path, poses_opt_path, additional_args, config_dir + "/odom_calib_config.txt")
	run_command(cmd)
...
def run_graphslam(carmen_path, log_path, output_dir, mode, config_dir):
	...
	cmd = "%s %s %s %s -o %s %s %s --save %s" % (program, log_path, PARAM_FILE, output_path, odom_calib, loops, args, config_dir + "/" + mode + "_config.txt")
	run_command(cmd)
...
def run_loop_closures(carmen_path, log_path, output_dir, mode, config_dir):
	...
	cmd = " %s %s %s -o %s -f %s --gps_id %d -i %s --start_paused 0 --save %s" % (program, log_path, PARAM_FILE, odom_calib, fused_odom, GPS_TO_USE, INTENSITY_MODE, config_dir + "/loops_" + mode + "_config.txt")
...
def main(log_path, skip_until):
	...
	if (not skip_until) or (skip_until <= 2):
		run_graphslam(carmen_path, log_path, output_dir, "fused_odom", config_dir)	

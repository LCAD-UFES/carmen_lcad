#include "laslam_interpolation.h"

template <typename M1, typename M2>
class LandmarkSlamMessagesInterpolation {

public:

	LandmarkSlamMessagesInterpolation(int list_size);
	...
	int Initialized() { return interpolation_list_initialized; }
	virtual ~LandmarkSlamMessagesInterpolation();

private:
	int interpolation_list_initialized;
...
template <typename M1, typename M2>
void LandmarkSlamMessagesInterpolation<M1, M2>::AddMessageToInterpolationList(M1 *message)
{
	...
	{
		this->interpolation_list_initialized = 1;
		this->interpolation_list_counter = 0;
		return;
	}
...
template <typename M1, typename M2>
M1 LandmarkSlamMessagesInterpolation<M1, M2>::InterpolateMessages(M2 *message)
{
	M1 nearest_message;
	int nearest_index = 0;
...
template <typename M1, typename M2>
LandmarkSlamMessagesInterpolation<M1, M2>::~LandmarkSlamMessagesInterpolation() {

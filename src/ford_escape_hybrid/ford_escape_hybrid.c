			...
			{   // PID
				...
				else if ((robot_model_name == ROBOT_NAME_ECOTECH4) || (robot_model_name == ROBOT_NAME_MPW700) || (robot_model_name == ROBOT_NAME_ASTRU))
				{
					...
					if (ford_escape_hybrid_config->nun_motion_commands > 1) //calcula tamanho do plano para passar para funcao de PID.
						plan_size = 0.5 * DIST2D(ford_escape_hybrid_config->current_motion_command_vector[0], ford_escape_hybrid_config->current_motion_command_vector[ford_escape_hybrid_config->nun_motion_commands - 1]);
					else
						plan_size = 0.0;

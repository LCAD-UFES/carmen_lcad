...
def make_parser():
    ...
    parser.add_argument('--device', default='cpu', help='cuda device, i.e. 0 or 0,1,2,3 or cpu') #mudei para CPU
    ...
    parser.add_argument('--image', type=float, help='archive numpy')
...
def detect(opt):
    ...
    dataset = LoadImages(opt.image, img_size=imgsz, stride=stride)
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        #img = opt.image #add this line 
        #device = torch.torch.device("cuda" if torch.cuda.is_available() else "cpu") #add this line
...
def main(image):
    ...
    opt.image = image
    opt.source = 'data/example.jpg'
    opt.device = 'cpu'

    with torch.no_grad(): #disable calc grad
        return detect(opt)

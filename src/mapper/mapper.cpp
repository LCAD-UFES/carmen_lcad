...
carmen_mapper_update_cells_bellow_semitrailer(carmen_point_t pose, carmen_map_t *map, double prob, int type, double beta)
{
	...
	for (int i = 0; i < col_n_points; i++)
	{
		collision_model_circles_center_in_world[4 * i] = (collision_model_circles[4 * i] - semi_trailer_config.semi_trailers.d - semi_trailer_config.semi_trailers.M) * cos(beta) + collision_model_circles[4 * i + 1] * sin(beta);
		collision_model_circles_center_in_world[4 * i + 1] = (collision_model_circles[4 * i] - semi_trailer_config.semi_trailers.d - semi_trailer_config.semi_trailers.M) * sin(beta) - collision_model_circles[4 * i + 1] * cos(beta);
	}
	for (int i = 0; i < col_n_points; i++)
	{
		collision_model_circles_center_in_map[4 * i] = (collision_model_circles_center_in_world[4 * i] + pose.x - map->config.x_origin) / map->config.resolution;

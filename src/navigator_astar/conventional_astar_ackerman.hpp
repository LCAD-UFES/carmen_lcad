...
public:
	...
	carmen_robot_and_trailers_traj_point_t carmen_conventional_astar_ackerman_kinematic(carmen_robot_and_trailers_traj_point_t point, double lenght, double phi, double v);
	void open_node(carmen_astar_node_p tree);
	void carmen_conventional_astar_ackerman_astar(carmen_robot_and_trailers_traj_point_t start, carmen_robot_and_trailers_traj_point_t goal,carmen_planner_path_p path, carmen_robot_ackerman_config_t *robot_conf);
	...
	int is_obstacle_1d(carmen_robot_and_trailers_traj_point_t point);
	double h_score(carmen_robot_and_trailers_traj_point_t point);
...
private:
	...
	carmen_robot_and_trailers_traj_point_t carmen_conventional_astar_ackerman_kinematic_2(carmen_robot_and_trailers_traj_point_t point, float lenght, float phi, float v);
	int rs_get_astar_path(int rs_pathl, carmen_robot_and_trailers_traj_point_t *points, carmen_planner_path_p path);
	void astar_init_parameters(carmen_robot_and_trailers_traj_point_t goal);

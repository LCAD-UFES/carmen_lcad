...
bool RobotBrainServiceService::start(int argc, char* argv[])
{
  ...
  //Create the topics
//  SimEventsUtils::createTopic(communicator(), "StraightEdgeFinderMessageTopic");
  ...
  while(!connected)
  {
    try
    {
      ...
      itsAdapter = communicator()->createObjectAdapterWithEndpoints("StraightEdgeFinder",
          adapterStr);
      connected = true;
    }
  ...
  //Create the manager and its objects
        itsMgr = new ModelManager("StraightEdgeFinderService");

  LINFO("Starting StraightEdgeFinder");
  nub::ref<StraightEdgeFinder> ret(new StraightEdgeFinder(*itsMgr, "StraightEdgeFinder1", "StraightEdgeFinder2"));
  LINFO("StraightEdgeFinder Created");
  itsMgr->addSubComponent(ret);
  LINFO("StraightEdgeFinder Added As Sub Component");
  ret->init(communicator(), itsAdapter);
  LINFO("StraightEdgeFinder Inited");

...
bool RobotBrainServiceService::start(int argc, char* argv[])
{
  ...
  //Create the topics
//  SimEventsUtils::createTopic(communicator(), "VisionBrainComponentMessageTopic");
  ...
  while(!connected)
  {
    try
    {
      ...
      itsAdapter = communicator()->createObjectAdapterWithEndpoints("VisionBrainComponent",
          adapterStr);
      connected = true;
    }
  ...
  //Create the manager and its objects
        itsMgr = new ModelManager("VisionBrainComponentService");

  LINFO("Starting VisionBrainComponent");
  nub::ref<VisionBrainComponentI> ret(new VisionBrainComponentI(*itsMgr, "VisionBrainComponent1", "VisionBrainComponent2"));
  LINFO("VisionBrainComponent Created");
  itsMgr->addSubComponent(ret);
  LINFO("VisionBrainComponent Added As Sub Component");
  ret->init(communicator(), itsAdapter);
  LINFO("VisionBrainComponent Inited");

...
bool RobotBrainServiceService::start(int argc, char* argv[])
{
  ...
  while(!connected)
  {
    try
    {
      ...
      itsAdapter = communicator()->createObjectAdapterWithEndpoints("BeeStem",
          adapterStr);
      connected = true;
    }
  ...
  LINFO("Starting BeeStem");
  nub::ref<BeeStemI> ret(new BeeStemI(0, *itsMgr, "BeeStem1", "BeeStem2"));
  LINFO("BeeStem Created");
  itsMgr->addSubComponent(ret);
  LINFO("BeeStem Added As Sub Component");
  ret->init(communicator(), itsAdapter);
  LINFO("BeeStem Inited");

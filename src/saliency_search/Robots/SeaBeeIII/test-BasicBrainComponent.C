...
bool RobotBrainServiceService::start(int argc, char* argv[])
{
  ...
  itsMgr = new ModelManager("BasicBrainComponentService");

  LINFO("Starting BasicBrainComponent");
  nub::ref<BasicBrainComponentI> bbc(new BasicBrainComponentI(0, *itsMgr, "BasicBrainComponent1", "BasicBrainComponent2"));
  LINFO("BasicBrainComponent Created");
  itsMgr->addSubComponent(bbc);
  LINFO("BasicBrainComponent Added As Sub Component");
  bbc->init(communicator(), itsAdapter);
  LINFO("BasicBrainComponent Inited");

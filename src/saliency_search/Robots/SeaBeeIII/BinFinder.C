...
// ######################################################################
void BinFinder::registerTopics()
{
  LINFO("Registering BinFinder Message");
  this->registerPublisher("BinFinderMessageTopic");
...
double BinFinder::angle( CvPoint* pt1, CvPoint* pt2, CvPoint* pt0 ) {
...
CvSeq* BinFinder::findSquares4( IplImage* img, CvMemStorage* storage ) {
  CvSeq* contours;
  int i, c, l, N = 11;
...
// ######################################################################
void BinFinder::updateFrame(Image<PixRGB<byte> > img, std::string cameraId)
{
...
bool BinFinder::inCorner(CvSeq* result)
{

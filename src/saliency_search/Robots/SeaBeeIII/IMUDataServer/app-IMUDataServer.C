...
bool RobotBrainServiceService::start(int argc, char* argv[])
{
  ...
  itsMgr = new ModelManager("IMUDataServerService");

  LINFO("Starting IMUDataServer");
  nub::ref<IMUDataServer> bbc(new IMUDataServer(0, *itsMgr, "IMUDataServer1", "IMUDataServer2"));
  LINFO("IMUDataServer Created");
  itsMgr->addSubComponent(bbc);
  LINFO("IMUDataServer Added As Sub Component");
  bbc->init(communicator(), itsAdapter);
  LINFO("IMUDataServer Inited");

...
#include "Robots/SeaBeeIII/StereoVisionI.H"
#include <Ice/Ice.h>
...
bool RobotBrainServiceService::start(int argc, char* argv[])
{
  ...
  itsMgr = new ModelManager("StereoVisionService"); 
  
  LINFO("Starting StereoVision");
  nub::ref<StereoVision> bbc(new StereoVision(0, *itsMgr, "StereoVision1", "StereoVision2"));
  LINFO("StereoVision Created");
  itsMgr->addSubComponent(bbc);
  LINFO("StereoVision Added As Sub Component");
  bbc->init(communicator(), itsAdapter); 
  LINFO("StereoVision Inited");

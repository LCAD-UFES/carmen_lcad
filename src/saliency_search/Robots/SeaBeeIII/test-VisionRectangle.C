...
bool RobotBrainServiceService::start(int argc, char* argv[])
{
  ...
  //Create the topics
//  SimEventsUtils::createTopic(communicator(), "VisionRectangleMessageTopic");
  ...
  //Create the manager and its objects
        itsMgr = new ModelManager("VisionRectangleService");

  LINFO("Starting VisionRectangle");
  nub::ref<VisionRectangle> ret
    (new VisionRectangle(*itsMgr, "VisionRectangle1", "VisionRectangle2"));
  LINFO("VisionRectangle Created");
  itsMgr->addSubComponent(ret);
  LINFO("VisionRectangle Added As Sub Component");
  ret->init(communicator(), itsAdapter);
  LINFO("VisionRectangle Inited");

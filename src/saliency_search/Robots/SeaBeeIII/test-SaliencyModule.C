...
//  SimEventsUtils::createTopic(communicator(), "SalientPointMessageTopic");
  ...
  while(!connected)
  {
    try
    {
      ...
      itsAdapter = communicator()->createObjectAdapterWithEndpoints("SaliencyModule", adapterStr);
      connected = true;
  ...
  nub::ref<SaliencyModuleI> salMod(new SaliencyModuleI(*itsMgr));
  itsMgr->addSubComponent(salMod);

  salMod->init(communicator(), itsAdapter);

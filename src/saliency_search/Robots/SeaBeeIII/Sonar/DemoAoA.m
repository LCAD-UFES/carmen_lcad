...
%% Target Details
target.distance = 3000;%2500+WAVELENGTH*10121; % [m]
...
for i=1:length(x)
  target.angle = i     * pi / 180 + pi/2; % [rad]
  generate_observations(target.distance, target.angle);
...
%imag(data.fft1(data.fft1_maxIndex)), real(data.fft1(data.fft1_maxIndex))
...
%  phase(2,i) = delta(2);
%  phase(3,i) = delta(3);
  phase(1,i) = data.phase1;
  phase(2,i) = data.phase2;
  phase(3,i) = data.phase3;
...
for i=1:length(x)
  ...
  switch best_phase_difference_index
    ...
    case 2 % 2 - 3
      ...
      y(i) = (phase_difference - 5 / 6 * pi) * ((WAVELENGTH / 2) / SENSOR_SPACING) * 180 / pi / 2;
    case 3 % 3 - 1
      if delta(2) > delta(1)
        phase_difference = -1 * delta(3) - 2 * pi;
      end

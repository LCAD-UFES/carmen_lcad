/*!@file Robots/Beobot2/Navigation/Gist_Navigation/app-Gist_Navigation.C
...
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/Robots/Beobot2/Navigation/Gist_Navigation/app-Gist_Navigation.C $
// $Id: app-Gist_Navigation.C 12962 2010-03-06 02:13:53Z irock $
//
...
bool RobotBrainServiceService::start(int argc, char* argv[])
{
  ...
  itsMgr = new ModelManager("Gist_NavigationService");

  LINFO("Starting Gist_Navigation System");
  nub::ref<Gist_Navigation>
    nav(new Gist_Navigation(*itsMgr, "Gist_Navigation", "Gist_Navigation"));
  LINFO("Gist_Navigation created");
  itsMgr->addSubComponent(nav);
  LINFO("Gist_Navigation Added As a subcomponent");
  nav->init(communicator(), itsAdapter);
  LINFO("Gist_Navigation initiated");

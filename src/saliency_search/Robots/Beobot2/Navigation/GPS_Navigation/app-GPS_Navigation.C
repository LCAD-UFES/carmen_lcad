/*!@file Robots/Beobot2/Navigation/GPS_Navigation/app-GPS_Navigation.C
...
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/Robots/Beobot2/Navigation/GPS_Navigation/app-GPS_Navigation.C $
// $Id: $
//
...
bool RobotBrainServiceService::start(int argc, char* argv[])
{
  ...
  itsMgr = new ModelManager("GPS_NavigationService");

  LINFO("Starting GPS_Navigation System");
  nub::ref<GPS_Navigation>
    nav(new GPS_Navigation(*itsMgr, "GPS_Navigation", "GPS_Navigation"));
  LINFO("GPS_Navigation created");
  itsMgr->addSubComponent(nav);
  LINFO("GPS_Navigation Added As a subcomponent");
  nav->init(communicator(), itsAdapter);
  LINFO("GPS_Navigation initiated");

/*!@file Robots2/Beobot2/Navigation/GistSal_Navigation/CornerNavigation.C */
// //////////////////////////////////////////////////////////////////// //
...
// $HeadURL: svn://ilab.usc.edu/trunk/saliency/src/Robots/Beobot2/CornerNavigation.C
// $ $Id: CornerNavigation.C 15310 2012-06-01 02:29:24Z itti $
//
//////////////////////////////////////////////////////////////////////////

#include "Robots/Beobot2/Navigation/GistSal_Navigation/CornerNavigation.H"
    ...
    while(fgets(line,sizeof line,dbconf)!= NULL)
      {
        ...
        if(line[0] != '#')
          {
            char subdb[256];
            char type;//Teach or Replay
            int start,end;
            int ret = sscanf(line,"%s %d %d %c",subdb,&start,&end,&type);
            if(ret == 4){
              LINFO("Got [%s],start[%d],end[%d],type[%c]",subdb,start,end,type);
              cornerDB tmpDB;
              tmpDB.path = sformat("%s/%s/%s/%s",FOLDER,DBNAME,subdb,IMAGE_PREFIX);
              tmpDB.start = start;
              tmpDB.end = end;
              tmpDB.type = type;
              loadDB(tmpDB);   
...
// ######################################################################
void CornerNavigation::start1()
{
  //if it runing in simulation
  if(!itsRealCamera.getVal())
    itsDBnum = 0;
  else // corner location to turn: initially set none
    itsDBnum = -1;
  LINFO("itsDBNUM: %d", itsDBnum);
}

// ######################################################################
void CornerNavigation::registerTopics()
{
  ...
  this->registerSubscription("CornerLocationMessageTopic");

  this->registerPublisher("CornerMatchMessageTopic");
  //this->registerPublisher("MotorRequestTopic");
}

// ######################################################################
void CornerNavigation::evolve()
{
      ...
      if(newImageFlag)
        {
          ...
          // check if we are turning a corner
          its_Corner_Loc_mutex.lock();
          int dbNum = itsDBnum;
          int status =  itsStatus;
          its_Corner_Loc_mutex.unlock();

          // if so navigate
          if(dbNum != -1 && status != C_PROCEDURE_INACTIVE)
            {
              LINFO("size: %" ZU , itsVisualObjectDBs.size() );
              int cornerSize = itsVisualObjectDBs[dbNum].vdb->numObjects();

            if(itsTeachImgID >= cornerSize)
              {
                itsStatus = C_PROCEDURE_INACTIVE;
              }
            else if(itsTeachImgID > int(cornerSize*0.9))
              {
                itsStatus = C_PROCEDURE_CONCLUDING;
              }
            else
              {
                itsStatus = C_PROCEDURE_ACTIVE;
              }
            navigation();
          }
        }
  ...
  if(!itsRealCamera.getVal() || newImageFlag)
    {
      drawState();
      itsOfs->writeRGB(itsDispImg, "CornerNav",
                       FrameInfo("CornerNav",SRC_POS));
    }
...
// ######################################################################
void CornerNavigation::recovery()
{
  int trytimes = 0;
  do{
    loadFrame();
    ...
    int nmatches =
      itsVisualObjectDBs[dbNum].vdb->getObjectMatches
...
void CornerNavigation::navigation()
{
  ...
  if(error != 0.0 && itsMatchList->size()!=0 &&
    ...
    {
      updateMotor(itsTransSpeed, itsRotSpeed,itsStatus);
      ...
      if(itsRealCamera.getVal())
        {
          ...
          itsDBnum  = -1;
          its_Corner_Loc_mutex.unlock();
          updateMotor(itsTransSpeed, itsRotSpeed, status);
        }
      else
        {
          updateMotor(itsTransSpeed, itsRotSpeed, 0);
          //recovery();
          //itsEstop = true; 
        }
    }
...
// ######################################################################
void CornerNavigation::loadDB(cornerDB db)
{

  //std::string path = sformat("%s/%s.sift",db.path.c_str(),IMAGE_PREFIX);

  // load the actual VisualObjectDatabase but don't load images
  db.vdb.reset(new VisualObjectDB());
  bool rt = db.vdb->loadFrom(sformat("%s.sift",db.path.c_str()),false);
  if(rt)
    {
      LINFO("Load SIFT Database");
      itsVisualObjectDBs.push_back(db);

      //Check if db is use for replay
      if(db.type =='R')
        itsReplayDBnum = int(itsVisualObjectDBs.size())-1;
    }
  else
    {
      LINFO("SIFT Database %s not exist, create new", db.path.c_str());
      saveDB(db.start,db.end,db.path.c_str());
    }
}

// ######################################################################
void CornerNavigation::saveDB(int start,int end,const std::string& path)
{
  cornerDB tmp;
  tmp.vdb.reset(new VisualObjectDB());
  ...
  tmp.vdb->saveTo(teachDB);
}

// ######################################################################
void CornerNavigation::loadFrame()
{
  ...
  //Check the size of subdb
  if(itsTeachImgID < (int)itsVisualObjectDBs[dbNum].vdb->numObjects())
    {
      itsTeachVo = itsVisualObjectDBs[dbNum].vdb->getObject(itsTeachImgID);
      itsTeachImg = itsTeachVo->getImage();
    }
  else
    {
      LINFO("=======Teach DB End=====");
  ...
  if(!itsRealCamera.getVal())
    {
      if(itsReplayImgID < (int)itsVisualObjectDBs[itsReplayDBnum].vdb->numObjects())
        {
          if(itsReplayImgID <(int)itsVisualObjectDBs[itsReplayDBnum].vdb->numObjects())
            {
              itsReplayVo = 
                itsVisualObjectDBs[itsReplayDBnum].vdb-> getObject(itsReplayImgID);	
              itsReplayImg = itsReplayVo->getImage(); 
              itsProcImg = itsReplayImg;
              itsReplayImgID++;
            }
        }
    }
  ...
  // running from real camera
  else
    {
      ...
      rutz::shared_ptr<VisualObject> vo2 (new VisualObject ("1", "", itsReplayImg));
      itsReplayVo = vo2;
    }
...
// ######################################################################
bool CornerNavigation::computeSIFT()
{
  itsTimer.reset();
...
// ######################################################################
double CornerNavigation::computeDirection2(int teachID)
{
  ...
  for(int id = teachID-2; id < teachID+10; id++)
    {
      ...
      rutz::shared_ptr<VisualObject> vo1;
        if(id < (int)itsVisualObjectDBs[dbNum].vdb->numObjects()){
      vo1 = itsVisualObjectDBs[dbNum].vdb->getObject(id);
        }
        else{
                LINFO("DB END");
                        return 0.0;
      ...
      LDEBUG("Best Match Size %d,",bestMatchSize);
      ...
      for (int i = 0; i < msize; i ++)
        {
          ...
          double ut = tstk->getX() - itsReplayVo->getImage().getWidth()/2;
          double ud = refk->getX() -  itsTeachVo->getImage().getWidth()/2;

          double vt = tstk->getY() - itsReplayVo->getImage().getHeight()/2;
          double vd = refk->getY() -  itsTeachVo->getImage().getHeight()/2;
  ...
  for (int i = 0; i < bestMatchSize; i ++)
    {
      ...
      stdy += dy * dy;
      avgdy+= dy;
      LDEBUG("Error! i=[%d] BestMatchSize[%d] "
  ...
  if(bestMatchSize != 0)
    {
      ...
      stdy /= bestMatchSize;
      stdy  = sqrt(stdy);
      LDEBUG("Error! BestMatchSize[%d]  avgdx[%f] avgdy[%f]",
  ...
  LDEBUG("stdx %f stdy %f",stdx,stdy);
  uint stdcount = 0;
  ...
  LDEBUG("False Error Rate %1.3f",itsFalseErrRate);
  if(itsFalseErrRate == 0.0)
    {
      LDEBUG("Error!!!! stdcount[%d]  avgerr[%f]",
            stdcount,avgerr);
      LDEBUG("Error!!!! bestMatchSize[%d]  stdx[%f] stdy[%f]",
...
// ######################################################################
double CornerNavigation::computeDirection()
{
  itsTimer.reset();
  int dirF = 0,dirL = 0,dirR = 0;
  double avgdx = 0.0,avgdy = 0.0;
  ...
  double avgerr = sqrt(avgdx*avgdx + avgdy*avgdy);
  double newAvgerr = sqrt(newAvgX*newAvgX + newAvgY*newAvgY);
  double df = (itsTeachImgID - itsReplayImgID );
  double angle = (newAvgX*(56.0/33.0))/df;
  //double angle = (newAvgX*(16.0/33.0));
  LDEBUG("StdError[%f],StdErrX[%f],StdErrY[%f],Frame[%1.0f]",
        stdeverr,sqrt(stdx),sqrt(stdy),df);
  LDEBUG("AvgErr[%f],AvgX[%f],AvgY[%f]",avgerr,avgdx,avgdy);
  LDEBUG("StdAvgErr[%f],StdAvgX[%f],StdAvgY[%f]",newAvgerr,newAvgX,newAvgY);
  LDEBUG("Forward[%d], Right[%d], Left[%d] Angle[%f]",dirF,dirR,dirL,angle);
  itsError.x = newAvgX;
  itsError.y = newAvgY;
  itsError.z = newAvgerr;
  //FIXXX
...
// ######################################################################
void CornerNavigation::updateKeyframe(double error)
{
  ...
  if (itsMatchList->checkSIFTaffine())
    {
      ...
      LDEBUG("SIFT SCALE SX[%f] SY[%f] Theta[%f] Str[%f]",sx,sy,theta,str);
...
//
// ######################################################################
int CornerNavigation::computeEntropy()
{
  return 1;
}

// ######################################################################
void CornerNavigation::drawState()
{
  its_Corner_Loc_mutex.lock();
  int dbNum = itsDBnum;
  its_Corner_Loc_mutex.unlock();

  uint w = itsProcImg.getWidth()*2, h = itsProcImg.getHeight()*2;
  //uint w = 320, h = 240;
  LINFO("TeachImg w=%d,h=%d, dbNum: %d", w, h, dbNum);
  ...
  if(dbNum != -1)
    {
      // replay image
      Image< PixRGB<byte> > replayImg = rescale(itsReplayImg,w,h);
      sprintf(buffer,"img%5d",itsReplayImgID);
      writeText(replayImg, Point2D<int>(5,5),
                buffer, PixRGB<byte>(255,255,255),
                PixRGB<byte>(0,0,0),SimpleFont::FIXED(8));
      inplacePaste(itsDispImg, replayImg, Point2D<int>(0, 2*h));

      // teach image
      Image< PixRGB<byte> > teachImg = rescale(itsTeachImg,w,h);
      sprintf(buffer,"img%5d",itsTeachImgID);
      writeText(teachImg, Point2D<int>(5,5),
                buffer, PixRGB<byte>(255,255,255),
                PixRGB<byte>(0,0,0), SimpleFont::FIXED(8));
      inplacePaste(itsDispImg, teachImg, Point2D<int>(0, h));

      // teach keypoint image
      //inplacePaste(itsDispImg, itsTeachVo->getKeypointImage(1.0) ,
      //             Point2D<int>(w, h));

      // replay keypoint image
      ...
      if(itsEstop)
        {
          ...
          LDEBUG("itsdispImg[%d,%d] mimg[%d,%d] pt[%d,%d]",
                 itsDispImg.getWidth(), itsDispImg.getHeight(),
                 mimg.getWidth(), mimg.getHeight(), int(w*1.5), h);
...
Image<PixRGB<byte> > CornerNavigation::drawInfoImg()
{
  char buffer[128];
  ...
  // return if currently not turning
  sprintf(buffer, "DB# :[%d] Status:[%d]", dbNum, status);
  writeText(dirImg, Point2D<int>(10,70),
            buffer, PixRGB<byte>(255,255,255), PixRGB<byte>(0,0,0),
            SimpleFont::FIXED(8));
  if (dbNum == -1) return dirImg;

  // heading direction
  if(itsDir == 0)    { sprintf(buffer, "||| "); }
  else if(itsDir > 0){ sprintf(buffer, "--> "); }
  else               { sprintf(buffer, "<-- "); }
...
// ######################################################################
void CornerNavigation::updatePosition(double turn)
{
  itsDirVector += turn;
...
// ######################################################################
void CornerNavigation::updateMessage
...
//     }
  ...
  if(eMsg->ice_isA("::BeobotEvents::GistSalMessage"))
  {
    ...
    Image<PixRGB<byte> > img = Ice2Image<PixRGB<byte> >(gistSalMsg->currIma);

    its_Curr_Img_mutex.lock();
    itsCurrImg = img;
    itsCurrImgID = gistSalMsg->RequestID;
    its_Curr_Img_mutex.unlock();
  ...
  // corner location message
  else if(eMsg->ice_isA("::BeobotEvents::CornerLocationMessage"))
    {
      BeobotEvents::CornerLocationMessagePtr clMsg =
        BeobotEvents::CornerLocationMessagePtr::dynamicCast(eMsg);
      LINFO("Turn on on corner %d", clMsg->cornerLocation);

      // check if the corner location exceed the maximum index
      int index = clMsg->cornerLocation;
      LINFO("Got Corner location Message Corner:[%d]",index);

      //only take new command if corner system is not running
      its_Corner_Loc_mutex.lock();
      int status = itsStatus;
      its_Corner_Loc_mutex.unlock();

      if(status == C_PROCEDURE_INACTIVE)
        {
          if(index >= (int)itsVisualObjectDBs.size()) index = -1;
          its_Corner_Loc_mutex.lock();
          itsStatus = C_PROCEDURE_ACTIVE;
          itsTeachImgID = 0;
          itsDBnum = index;
...
// ######################################################################
void CornerNavigation::updateMotorPID(double tran, double rot,double error)
{
  error/=100.0;
  if(itsDir == 1)
    error *= -1.0;
  ...
  updateMotor(tran,pid,itsStatus);
}

// // ######################################################################
// void CornerNavigation::updateMotor(double tran, double rot)
// {
...
// ######################################################################
void CornerNavigation::updateMotor(double tran, double rot, int status)
{
  BeobotEvents::CornerMatchMessagePtr msg =
    new BeobotEvents::CornerMatchMessage;
  msg->transVel = tran;
  msg->rotVel   = rot;
  msg->status   = status;
  ...
  this->publish("CornerMatchMessageTopic", msg);
  LINFO("[%d] Publish corner match message Trans %f Rotation %f num: %d",
        itsPrevProcImgID, tran, rot, numMatches);
}

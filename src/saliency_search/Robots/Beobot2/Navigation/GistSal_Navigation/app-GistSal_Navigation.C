/*!@file Robots/Beobot2/Navigation/GistSal_Navigation/app-GistSal_Navigation.C
...
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/Robots/Beobot2/Navigation/GistSal_Navigation/app-GistSal_Navigation.C $
// $Id: app-GistSal_Navigation.C 12962 2010-03-06 02:13:53Z irock $
//
...
bool RobotBrainServiceService::start(int argc, char* argv[])
{
  ...
  itsMgr = new ModelManager("GistSal_NavigationService");

  LINFO("Starting GistSal_Navigation System");
  nub::ref<GistSal_Navigation>
    gsn(new GistSal_Navigation(*itsMgr, "GistSal_Navigation", "GistSal_Navigation"));
  LINFO("GistSal_Navigation created");
  itsMgr->addSubComponent(gsn);
  LINFO("GistSal_Navigation Added As a subcomponent");
  gsn->init(communicator(), itsAdapter);
  LINFO("GistSal_Navigation initiated");
  ...
  // link the environment to the GistSal navigation module
  gsn->setEnvironment(env);

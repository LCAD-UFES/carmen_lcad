/*!@file Robots/Beobot2/LaneFollowing/TV_Lane/app-TV_Lane.C
...
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/Robots/Beobot2/LaneFollowing/TV_Lane/app-TV_Lane.C $
// $Id: app-TV_Lane.C 12962 2010-03-06 02:13:53Z irock $
//
...
bool RobotBrainServiceService::start(int argc, char* argv[])
{
  ...
  itsMgr = new ModelManager("TV_LaneService");

  LINFO("Starting TV_Lane System");
  nub::ref<TV_Lane>
    nav(new TV_Lane(*itsMgr, "TV_Lane", "TV_Lane"));
  LINFO("TV_Lane created");
  itsMgr->addSubComponent(nav);
  LINFO("TV_Lane Added As a subcomponent");
  nav->init(communicator(), itsAdapter);
  LINFO("TV_Lane initiated");

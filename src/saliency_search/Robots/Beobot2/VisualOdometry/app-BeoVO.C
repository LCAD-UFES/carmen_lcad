/*!@file Robots/Beobot2/Hardware/app-BeoVO.C ICE module takes 
...
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/Robots/Beobot2/Hardware/app-BeoVO.C $
// $Id: app-BeoVO.C 15181 2012-02-28 03:21:09Z beobot $
//
...
#endif	
{
  ...
  //Create the topics
//  SimEventsUtils::createTopic(communicator(), "BeoVOMessageTopic");
  ...
  itsMgr = new ModelManager("BeoVOService");

  LINFO("Starting BeoVO System");
  nub::ref<BeoVO> cam
    (new BeoVO(*itsMgr, "BeoVO", "BeoVO"));
  LINFO("BeoVO created");
  itsMgr->addSubComponent(cam);
  LINFO("BeoVO Added As a subcomponent");
  cam->init(communicator(), itsAdapter);
  LINFO("BeoVO initiated");

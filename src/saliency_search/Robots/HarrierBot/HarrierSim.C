/*!@file BeoSub/HarrierSim.C Sub Simulator */
...
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/Robots/HarrierBot/HarrierSim.C $
// $Id: HarrierSim.C 12962 2010-03-06 02:13:53Z irock $
//

#include "Robots/HarrierBot/HarrierSim.H"
...
namespace {
  ...
  void nearCallback (void *data, dGeomID o1, dGeomID o2){
    HarrierSim *subSim = (HarrierSim *)data;
...
// ######################################################################
HarrierSim::HarrierSim(OptionManager& mgr, const std::string& descrName,
    const std::string& tagName, bool showWorld) :
  ModelComponent(mgr, descrName, tagName),
  itsHarrierLength(7.0), //the length of sub in meters
  itsHarrierWidth(4.0), //the radius of the sub in meters
  itsHarrierWeight(30), // the weight of the sub in kg
  vp(new ViewPort("HarrierSim")),
  itsPanThruster(0),
  itsPitchThruster(0),
  itsRollThruster(0),
  itsUpThruster(0),
...
void HarrierSim::drawHarrier()
{

//  vp->dsSetColor(1,1,0);

  vp->dsDraw3DSObject(dBodyGetPosition(itsHarrierBody),
      dBodyGetRotation(itsHarrierBody),
      itsHarrierObject);

  //double sides[3] = {itsHarrierWidth, itsHarrierLength, 0.5};
  //vp->dsDrawBox(
  //    dBodyGetPosition(itsHarrierBody),
  //    dBodyGetRotation(itsHarrierBody),
  //    sides);

}

//void HarrierSim::updateSensors(const double *pos, const double *R)
void HarrierSim::updateSensors(const dReal *pos, const dReal *R)
{
  itsXPos = pos[0];
  itsYPos = pos[1];
  itsDepth = pos[2];
...
Image<PixRGB<byte> > HarrierSim::getFrame(int camera)
{
  const dReal *bodyPos = dBodyGetPosition(itsHarrierBody);
  const dReal *bodyR = dBodyGetRotation(itsHarrierBody);
...
void HarrierSim::getSensors(float &xPos, float &yPos, float &depth,
...
void HarrierSim::setThrusters(float panThruster, float pitchThruster, float rollThruster, float upThruster)
{

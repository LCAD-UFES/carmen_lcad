%/*!@file Robots/matlab/localization/ekf_bearing.m  */
%
%
%// //////////////////////////////////////////////////////////////////// //
...
for i=1:nLandmarks
	Landmarks_x(i) = 1 - 2*rand;
	Landmarks_y(i) = 1 - 2*rand;
end
...
for i=1:100
  ...
  G = diag([5; 1],0); %The input noise, Gamma
  ...
  for li=1:size(bearings,1)
          ...
          Q = diag([0.001]);
          H = [ (Landmarks_y(li) - State_hat(yState)) / (belifeRange^2) ...
              -1*(Landmarks_x(li) - State_hat(xState)) / (belifeRange^2)  ...
              1];
  ...
  h = drawEllipse(State_hat, Cov_hat,10);
  if(~isempty(h))
		set(h,'color','r');
  end;
  pause

  ...
  for k=1:nParticles
    ...
    for li=1:size(landmarksIDs,1)
      ...
      if (isempty(landmarkPos))
        ...
        landmark_x = currentPos(xState) + (ranges(li)*cos(bearings(li) + currentPos(thState)));
        landmark_y = currentPos(yState) + (ranges(li)*sin(bearings(li) + currentPos(thState)));
        ...
        %belifeBearing = pi_to_pi(belifeBearing);

        %Compute the mesuement jacobian 
        %G = [ (landmarkPos(1) - currentPos(xState))/belifeRange   (landmarkPos(2) - currentPos(yState))/belifeRange ;
        %      (currentPos(yState) - landmarkPos(2))/belifeRange^2 (currentPos(xState) - landmarkPos(1))/belifeRange^2 ];
        ...
        Q = G*landmarkCov*G' + R;
        %K = landmarkCov*G*inv(Q);
        
        %plot(landmarkPos(1), landmarkPos(2), 'b*');
        landmarkPos = landmarkPos + (K*Innov)';
        landmarkCov = landmarkCov - W1*W1';
        %landmarkCov = (eye(size(landmarkCov)) - K*G)*landmarkCov; 
        %landmarkCov = diag([10 10]);
     
        %plot(landmarkPos(1), landmarkPos(2), 'k*');

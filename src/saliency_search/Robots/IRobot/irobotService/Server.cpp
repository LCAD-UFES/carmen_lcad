...
int run(int argc, char* argv[], const Ice::CommunicatorPtr& communicator)
{
  Ice::ObjectAdapterPtr adapter =
    communicator->createObjectAdapterWithEndpoints("IRobotAdapter", "tcp -p 10000");
  Ice::ObjectPtr object = new IRobotI(1);
  adapter->add(object, communicator->stringToIdentity("IRobotService"));
  adapter->activate();

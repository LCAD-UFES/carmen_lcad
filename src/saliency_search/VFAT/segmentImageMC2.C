/*!@file VFAT/segmentImageMC2.C Basic image segmenter blob finder using color */
...
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/VFAT/segmentImageMC2.C $
// $Id: segmentImageMC2.C 10794 2009-02-08 06:21:09Z itti $
//
...
#ifndef SEGMENTIMAGEMC2_C_DEFINED
#define SEGMENTIMAGEMC2_C_DEFINED
...
segmentImageMC2<SI_TEMPLATE>::segmentImageMC2()
{
...
void segmentImageMC2<SI_TEMPLATE>::SIsetVal(const typename
                                           ...
                                           std::vector<FLOAT> &thresh)
{
  ...
  typename std::vector<FLOAT>::const_iterator ival    = val.begin();
  typename std::vector<FLOAT>::const_iterator ithresh = thresh.begin();
...
template SI_TEMPLATE_CLASS inline
void segmentImageMC2<SI_TEMPLATE>::SIsetValThresh(const typename
                                                 ...
                                                 std::vector<FLOAT> &low)
{
  ...
  typename std::vector<FLOAT>::iterator ilow  = SI_lowThresh.begin();
  typename std::vector<FLOAT>::iterator ihigh = SI_highThresh.begin();

  // Check to make sure low is less than high for each feature
  while(ilow != SI_lowThresh.end())
  {
    if(*ilow >= *ihigh)
    {
      LFATAL("Low threshold must be less than high theshold. Got L %f H %f",*ilow,*ihigh);
    }
    ++ilow; ++ihigh;
  }
...
template SI_TEMPLATE_CLASS inline
void segmentImageMC2<SI_TEMPLATE>::SIresetCandidates(const bool whichWay)
{
  Image<bool>::iterator
...
template SI_TEMPLATE_CLASS inline
void segmentImageMC2<SI_TEMPLATE>::SIsetFrame(int *x, int *y)
{
...
template SI_TEMPLATE_CLASS inline
void segmentImageMC2<SI_TEMPLATE>::SIsetAvg(const INT doAvg)
{
...
template SI_TEMPLATE_CLASS inline
void segmentImageMC2<SI_TEMPLATE>::SIresetAvg()
{
  typename std::vector<FLOAT> temp(SI_channels,0);
...
/*********************************************************************/
template SI_TEMPLATE_CLASS
void segmentImageMC2<SI_TEMPLATE>::SIsegment(
                               Image<PixRGB<byte> > *image,
                               typename std::vector<Image<FLOAT> > *featureMap,
                               const bool lowPass)
{
  SI_workImage      = image;
  SIsegment(featureMap,lowPass);
}
                               ...
                               const bool lowPass)
{
  ...
  // run doSegment which will call several consecutive methods
  SIdoSegment();
...
/*********************************************************************/
template SI_TEMPLATE_CLASS
void segmentImageMC2<SI_TEMPLATE>::SItoggleCandidateBandPass(const bool toggle)
{
...
/*********************************************************************/
template SI_TEMPLATE_CLASS
void segmentImageMC2<SI_TEMPLATE>::SItoggleRemoveSingles(const bool toggle)
{
  SI_removeSingles = toggle;
}

/*********************************************************************/
template SI_TEMPLATE_CLASS
void segmentImageMC2<SI_TEMPLATE>::SIsetKillValue(const unsigned int kv)
{
  SI_killVal = kv;
}
...
*/
template SI_TEMPLATE_CLASS
Image<INT> segmentImageMC2<SI_TEMPLATE>::SIcreateMother(const Image<INT> &img) const
{
  Image<INT> mother;
 ...
 */
template SI_TEMPLATE_CLASS
Image<int> segmentImageMC2<SI_TEMPLATE>::SIreturnBlobs() const
{
  return SI_blobID;
}
...
template SI_TEMPLATE_CLASS
Image<bool> segmentImageMC2<SI_TEMPLATE>::SIreturnCandidates() const
{
...
template SI_TEMPLATE_CLASS
Image<FLOAT> segmentImageMC2<SI_TEMPLATE>::SIreturnNormalizedCandidates() const
{
  Image<FLOAT> NC;
...
template SI_TEMPLATE_CLASS
Image<PixRGB<FLOAT> > segmentImageMC2<SI_TEMPLATE>::SIreturnWorkImage() const
{
...
template SI_TEMPLATE_CLASS
INT segmentImageMC2<SI_TEMPLATE>::SInumberBlobs() const
{
...
template SI_TEMPLATE_CLASS
std::vector<INT> segmentImageMC2<SI_TEMPLATE>::SIgetBlobMap() const
{
...
template SI_TEMPLATE_CLASS
void segmentImageMC2<SI_TEMPLATE>::SIcalcMassCenter()
{

  Image<int>::iterator iblobID           = SI_blobID.beginw();
  Image<bool>::iterator icandidatePixels = SI_candidatePixels.beginw();

  const ushort width  = (ushort)SI_candidatePixels.getWidth();
  const ushort height = (ushort)SI_candidatePixels.getHeight();
...
template SI_TEMPLATE_CLASS
FLOAT segmentImageMC2<SI_TEMPLATE>::SIgetCenterX(const INT blob) const
{
...
template SI_TEMPLATE_CLASS
FLOAT segmentImageMC2<SI_TEMPLATE>::SIgetCenterY(const INT blob) const
{
...
template SI_TEMPLATE_CLASS
INT segmentImageMC2<SI_TEMPLATE>::SIgetMass(const INT blob) const
{
  return SI_mass[blob];
}
...
template SI_TEMPLATE_CLASS
int segmentImageMC2<SI_TEMPLATE>::SIgetXmin(const INT blob) const
{
  return SI_xmin[blob];
}
...
template SI_TEMPLATE_CLASS
int segmentImageMC2<SI_TEMPLATE>::SIgetXmax(const INT blob) const
{
  return SI_xmax[blob];
}
...
template SI_TEMPLATE_CLASS
int segmentImageMC2<SI_TEMPLATE>::SIgetYmin(const INT blob) const
{
  return SI_ymin[blob];
}
...
template SI_TEMPLATE_CLASS
int segmentImageMC2<SI_TEMPLATE>::SIgetYmax(const INT blob) const
{
  return SI_ymax[blob];
}
...
template SI_TEMPLATE_CLASS
int segmentImageMC2<SI_TEMPLATE>::SIgetImageSizeX() const
{
...
template SI_TEMPLATE_CLASS
int segmentImageMC2<SI_TEMPLATE>::SIgetImageSizeY() const
{
...
/*********************************************************************/
template SI_TEMPLATE_CLASS inline
void segmentImageMC2<SI_TEMPLATE>::SIgetValue(INT *blob,
...
template SI_TEMPLATE_CLASS
void segmentImageMC2<SI_TEMPLATE>::SIgetValueMean(INT *blobListSize,
...
*/
template SI_TEMPLATE_CLASS inline
void segmentImageMC2<SI_TEMPLATE>::SIfindCandidates()
{
  // FIX
  std::vector<int>::iterator imasterVec = SI_masterVec.begin();
...
*/
template SI_TEMPLATE_CLASS inline
void segmentImageMC2<SI_TEMPLATE>::SIfindCandidatesNoBandPass()
{
  // FIX
  std::vector<int>::iterator imasterVec = SI_masterVec.begin();
        ...
        ++ilowThresh, ++ihighThresh)
  {
    ...
    while(iifeatureMaps != ifeatureMaps->endw())
    {
      //LINFO("Feature %f, high %f, low %f",*iifeatureMaps,*ihighThresh,*ilowThresh);
      if((*iifeatureMaps > *ihighThresh) || (*iifeatureMaps < *ilowThresh))
        *candidatePixelsIter = false;
...
*/
template SI_TEMPLATE_CLASS inline
void segmentImageMC2<SI_TEMPLATE>::SIremoveSingles()
{
  Image<bool>::iterator icandidatePixels = SI_candidatePixels.beginw();
...
*/
template SI_TEMPLATE_CLASS inline
void segmentImageMC2<SI_TEMPLATE>::SIremoveSinglesItr()
{
  Image<bool>::iterator icandidatePixels = SI_candidatePixels.beginw();
  ...
  for(unsigned int y = 0; y < height; y++)
  {
    for(unsigned int x = 0; x < width; x++)
    {
      if(*icandidatePixels)
      {
        unsigned int kill = 0;
        if(x != 0)
          if(*icandidatePixelsLeft)
            kill++;
        if(x < width - 1)
          if(*icandidatePixelsRight)
            kill++;
        if(y != 0)
          if(*icandidatePixelsTop)
            kill++;
        if(y < height - 1)
          if(*icandidatePixelsBottom)
            kill++;
        if(kill < SI_killVal)
          *icandidatePixels = false;
      }
      ...
      ++icandidatePixels;
      ++icandidatePixelsBottom; ++icandidatePixelsRight;
...
*/
template SI_TEMPLATE_CLASS inline
void segmentImageMC2<SI_TEMPLATE>::SIdiscreteLinking()
{
  const ushort width    = (ushort)SI_candidatePixels.getWidth();
  const ushort height   = (ushort)SI_candidatePixels.getHeight();
  bool trace            = false;
      ...
      if(SI_candidatePixels.getVal(x,y))
      {
        ...
        if(x > 0) // 1 or 0?
        {
          if(SI_candidatePixels.getVal(x-1,y))
          {
            ...
            const INT check =  SI_blobID.getVal((x-1),y);
  ...
  LINFO("Candidate pixels recognized %d",pixID);
  SI_num = pixID;
}
...
*/
template SI_TEMPLATE_CLASS inline
void segmentImageMC2<SI_TEMPLATE>::SIdiscreteLinkingOrtho()
{
  const int width       = SI_candidatePixels.getWidth();
  const int height      = SI_candidatePixels.getHeight();
  bool trace            = false;
  ...
  Image<int>::iterator  blobIDItr          = SI_blobID.beginw();
  ...
  for(int y = 0; y < height; y++)
  {
    ...
    for(int x = 0; x < width; x++)
    {
      // This is: if(SI_candidatePixels.getVal(x,y))
      if(*candidatePixelsItr++)
      {
        // Start a new pixel row group
        if(!trace)
        {
          pixID++;
          trace        = true;
        ...
        // This is: SI_blobID.setVal(x,y,pixID);
        *blobIDItr++ = pixID;
        if(y > 0) // 1 or 0?
        {
          //This is: if(SI_candidatePixels.getVal(x,y-1))
          if(*(candidatePixelsItr - (1 + width)))
          {
            // relink blob pixels if needed
            //This is: const INT check =  SI_blobID.getVal(x,y-1);
            const INT check = *(blobIDItr - (1 + width));
            if((signed)check != lastNeighbor)
            {
              SIbackwardLink(pixID,check);
              lastNeighbor = check;
      ...
      {
        trace        = false;
        //This is: SI_blobID.setVal(x,y,0);
        *blobIDItr++ = 0;
        lastNeighbor = -2;
      }
    }
...
*/
template SI_TEMPLATE_CLASS inline
void segmentImageMC2<SI_TEMPLATE>::SIbackwardLink(const INT slave,
                                                  const INT master)
{
  ...
  if(SI_masterVec[master] == -1)
  {
    // I have no master
    if(SI_masterVec[slave] == -1)
    {
      //LINFO("my master has no master/I have no master");
      SI_masterVec[master] = SI_masters;
      SI_masterVec[slave]  = SI_masters;
      SI_masters++;
      SI_mastersCount++;
    }
    ...
    else
    {
      ...
      SI_masterVec[master] = SI_masterVec[slave];
    }
  }
  ...
  else
  {
    ...
    if(SI_masterVec[slave] == -1)
    {
      ...
      SI_masterVec[slave] = SI_masterVec[master];
    }
    // I already have a master
    else
    {
      ...
      std::vector<int>::iterator masterVecItr = SI_masterVec.begin();
      for(INT i = 0; i <= SI_maxIDVal; i++, ++masterVecItr)
      {
        if(*masterVecItr == SI_masterVec[slave])
        {
          //LINFO("SET %d to %d",masterVec[i],masterVec[slave]);
          *masterVecItr = SI_masterVec[master];
        }
      }
      SI_masterVec[slave] = SI_masterVec[master];
      SI_mastersCount--;
...
/*********************************************************************/
template SI_TEMPLATE_CLASS inline
void segmentImageMC2<SI_TEMPLATE>::SIbasicLink(const INT node)
{
  if(node > SI_maxIDVal)
    SI_maxIDVal = node;

  // my master has no master
  if(SI_masterVec[node] == -1)
  {
    //LINFO("my master has no master/I have no master");
    SI_masterVec[node] = SI_masters;
    SI_masters++;
    SI_mastersCount++;
  }
}
...
template SI_TEMPLATE_CLASS inline
void segmentImageMC2<SI_TEMPLATE>::SIcombine()
{
  SI_totalBlobs = 0;
  ...
  while(candidatePixelsItr != SI_candidatePixels.endw())
  {
    if(*candidatePixelsItr++)
      *blobIDItr = SI_masterVec[*blobIDItr];
    blobIDItr++;
  }
  ...
  */
  ...
  for(INT x = 0; x < SI_num; x++, ++masterVecItr)
  {
    ...
    {
      if((int)SI_reOrderVec[y] == *masterVecItr)
        add = false;
    }
...
template SI_TEMPLATE_CLASS inline
void segmentImageMC2<SI_TEMPLATE>::SIdoSegment()
{
...
template class segmentImageMC2<float, unsigned int, 1>;
template class segmentImageMC2<float, unsigned int, 2>;
template class segmentImageMC2<float, unsigned int, 3>;
template class segmentImageMC2<float, unsigned int, 4>;

/*!@file Channels/JunctionChannel.C */
...
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/Channels/JunctionChannel.C $
// $Id: JunctionChannel.C 14356 2011-01-04 21:30:52Z dberg $
//

#ifndef JUNCTIONCHANNEL_C_DEFINED
#define JUNCTIONCHANNEL_C_DEFINED
...
// ######################################################################
// junction Channel member definitions:
// ######################################################################
JunctionChannel::JunctionChannel(OptionManager& mgr,
  ...
  SingleChannel(mgr, "JunctionChannel", "JunctionChannel", vs,
                rutz::shared_ptr< PyrBuilder<float> >(NULL)),
  itsFull(vs == LJUNCTION ? &OPT_LFullImplementation :
          vs == TJUNCTION ? &OPT_TFullImplementation :
          vs == XJUNCTION ? &OPT_XFullImplementation :
          vs == ENDPOINT ? &OPT_EFullImplementation :
          (ModelOptionDef*) 0,
          this),  // see Channels/ChannelOpts.C
  itsDelta(vs == LJUNCTION ? &OPT_LJunctionChannelDelta :
           vs == TJUNCTION ? &OPT_TJunctionChannelDelta :
           vs == XJUNCTION ? &OPT_XJunctionChannelDelta :
           vs == ENDPOINT ? &OPT_EndPointChannelDelta :
           (ModelOptionDef*) 0,
           this),
  itsOriChan(oc),
...
GVX_TRACE(__PRETTY_FUNCTION__);
  ...
  setDescriptiveName(sformat("Junction(%d%d%d%d%d%d%d%d)",
                             r0, r1, r2, r3, r4, r5, r6, r7));

  setTagName(sformat("junction_%d%d%d%d%d%d%d%d",
                     r0, r1, r2, r3, r4, r5, r6, r7));
}
...
// ######################################################################
void JunctionChannel::doInput(const InputFrame& inframe)
{
GVX_TRACE(__PRETTY_FUNCTION__);
  // access the orientation channel
  ASSERT(itsOriChan.get() != 0);
  const uint num = itsOriChan->getModelParamVal<uint>("NumOrientations");
  ASSERT(num == 4);
  ...
  //LINFO("Junction Delta %d",itsDelta.getVal());
  ...
  if (itsFull.getVal())
  {
    for (uint i = 0; i < ori0_pyr.size(); i ++)
      result[i] = junctionFilterFull(ori0_pyr[i],  ori45_pyr[i],
  ...
  {
    for (uint i = 0; i < ori0_pyr.size(); i ++)
      result[i] = junctionFilterPartial(ori0_pyr[i],  ori45_pyr[i],
                                        ori90_pyr[i], ori135_pyr[i],
                                        R, dx, dy);
  }
...
#endif // JUNCTIONCHANNEL_C_DEFINED

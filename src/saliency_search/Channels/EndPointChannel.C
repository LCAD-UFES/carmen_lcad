/*!@file Channels/EndPointChannel.C */
...
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/Channels/EndPointChannel.C $
// $Id: EndPointChannel.C 9503 2008-03-19 23:10:33Z rjpeters $
//

#ifndef ENDPOINTCHANNEL_C_DEFINED
#define ENDPOINTCHANNEL_C_DEFINED
...
// ######################################################################
// End point Channel member definitions:
// ######################################################################
EndPointChannel::EndPointChannel(OptionManager& mgr,
                                 nub::soft_ref<OrientationChannel> oc)
  :
  ComplexChannel(mgr, "EndPoint", "end-point", ENDPOINT),
  itsNumOrients(&OPT_NumEOrients, this), // see Channels/ChannelOpts.{H,C}
  itsOriChan(oc)
{
...
// ######################################################################
void EndPointChannel::doInput(const InputFrame& inframe)
{
GVX_TRACE(__PRETTY_FUNCTION__);
  for (uint i = 0; i < numChans(); ++i)
    {
      subChan(i)->input(inframe);
      LINFO("EndPoint pyramid (%d/%d) ok.", i+1, numChans());
    }
}
...
#endif // ENDPOINTCHANNEL_C_DEFINED

/*!@file Channels/LJunctionChannel.C */
...
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/Channels/LJunctionChannel.C $
// $Id: LJunctionChannel.C 9503 2008-03-19 23:10:33Z rjpeters $
//

#ifndef LJUNCTIONCHANNEL_C_DEFINED
#define LJUNCTIONCHANNEL_C_DEFINED
...
// ######################################################################
// L Junction Channel member definitions:
// ######################################################################
LJunctionChannel::LJunctionChannel(OptionManager& mgr,
                                   nub::soft_ref<OrientationChannel> oc)
  :
  ComplexChannel(mgr, "LJunction", "l-junction", LJUNCTION),
  itsNumOrients(&OPT_NumLOrients, this), // see Channels/ChannelOpts.{H,C}
  itsOriChan(oc)
{
...
// ######################################################################
LJunctionChannel::~LJunctionChannel()
{
GVX_TRACE(__PRETTY_FUNCTION__);
}

// ######################################################################
void LJunctionChannel::buildSubChans()
{
GVX_TRACE(__PRETTY_FUNCTION__);
...
// ######################################################################
void LJunctionChannel::doInput(const InputFrame& inframe)
{
GVX_TRACE(__PRETTY_FUNCTION__);
  for (uint i = 0; i < numChans(); ++i)
    {
      subChan(i)->input(inframe);
      LINFO("LJunction pyramid (%d/%d) ok.", i+1, numChans());
    }
}
...
#endif // LJUNCTIONCHANNEL_C_DEFINED

/*!@file Util/jenkinshash.C 32-bit hash functions (NOT CRYPTOGRAPHIC!) */

// This code here is almost verbatim from
// http://burtleburtle.net/bob/c/lookup2.c (see original public domain
// notice below) with only trivial modifications to use ANSI C
// prototypes instead of K&R prototypes

//
// Primary maintainer for this file: Rob Peters <rjpeters at usc dot edu>
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/Util/jenkinshash.C $
// $Id: jenkinshash.C 14376 2011-01-11 02:44:34Z pez $
//

#ifndef UTIL_JENKINSHASH_C_DEFINED
#define UTIL_JENKINSHASH_C_DEFINED
...
ub4 hash(const ub1* k,      /* the key */
         const ub4 length,  /* the length of the key */
...
--------------------------------------------------------------------
*/
ub4 hash2(const ub4 *k,       /* the key */
          const ub4  length,  /* the length of the key, in ub4s */
          const ub4  initval) /* the previous hash, or an arbitrary value */
{
   ub4 a,b,c,len;
  ...
  for (h=0, b=buf+1; h<8; ++h, ++b)
  {
    for (i=0; i<MAXLEN; ++i)
    {
      ...
      *(b-1)=(ub1)~0;
      ...
      if ((ref != x) || (ref != y))
      {
        printf("alignment error: %x %x %x %d %d\n",ref,x,y,h,i);
      }
    }
  }
}
...
void driver4()
{
  ...
  {
    h = hash(buf, (ub4)0, h);
    printf("%2d  0-byte strings, hash is  %x\n", i, h);
  }
}
...
#endif // UTIL_JENKINSHASH_C_DEFINED

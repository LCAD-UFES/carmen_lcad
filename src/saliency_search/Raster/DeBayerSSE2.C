/*!@file Raster/DebayerSSE2.C is the debayer class with sse2 */
...
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/Raster/DeBayerSSE2.C $
// $Id: DeBayerSSE2.C 10794 2009-02-08 06:21:09Z itti $
//
#include <stdio.h>
#include <stdint.h>
#include <emmintrin.h>
...
debayerSSE2 (const Image<T>& src1, BayerFormat format)
{
#ifndef INVT_USE_SSEDB
  LFATAL("you must have SSE2 support");
  return Image<PixRGB<T> >();
#else
  //! althogh we use a template here but actuaaly we only support the 8 bit depth  mode
  ...
  if ((src1.getWidth() % 32) != 0)
    {
      patchWidth = 32 - (src1.getWidth() % 16);
  ...
  if(bayerInterpolateTo_8u_bgra_sse2 (planes,plane_stride,
                                   bgra_img, bgra_stride,
                                      width, height, format) < 0)
    LFATAL("error in debayer with sse2");
  ...
  return res;
#endif //INVT_USE_SSEDB
}

int
bayerInterpolateTo_8u_bgra_sse2 (uint8_t ** src, int sstride,
  ...
  return -1;
    ...
    for (i = 0; i < 4; i++) {
        if (!IS_ALIGNED16(src[i]) || !IS_ALIGNED16(sstride)) {
          LERROR("%s: src[%d] is not 16-byte aligned", __FUNCTION__, i);
            return -1;
        }
    }
    if (!IS_ALIGNED16(dst) || !IS_ALIGNED128(dstride)) {
      LERROR("%s: dst is not 16-byte aligned or 128-byte stride aligned", __FUNCTION__);
        return -1;
    }
...
template Image<PixRGB<byte> >  debayerSSE2(const Image<byte>& src, BayerFormat format);
template Image<PixRGB<uint16> >  debayerSSE2(const Image<uint16>& src, BayerFormat format);

/*!@file Surprise/test-surpriseFFT.C test basic behavior of SurpriseMap and contents */
...
// $HeadURL: svn://isvn.usc.edu/software/invt/trunk/saliency/src/Surprise/test-surpriseFFT.C $
// $Id: test-surpriseFFT.C 7924 2007-02-15 18:13:05Z rjpeters $
//
...
{
#ifndef HAVE_FFTW3_H
  LFATAL("you must have fftw3 installed to use this program\n");
#else
  if (argc != 3) LFATAL("USAGE: %s <image.pgm> <variance.pfm>", argv[0]);
  ...
  Image<double> var = Raster::ReadFloat(argv[2]);
  ...
  Image<double> inZero(input.getDims(), NO_INIT);
  ...
  for (int i = 0; i < NITER; i ++)
    {
      ...
      if(i >= SURPRISE)
      {
        ...
        {
          LINFO("SWITCH");
          surp = smap.surprise(altSample,inZero,var);
        }
        else
        {
          surp = smap.surprise(sample,in,var);
        }
      }
      else
      {

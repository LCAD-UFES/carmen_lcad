...
void
Detector::update_moving_object_velocity(carmen_robot_and_trailers_traj_point_t &robot_pose)
{
	int count = 0;
	for (int i = MOVING_OBJECT_HISTORY_SIZE - 2; i >= 0 ; i--)
	{
		...
		{
			double dist = carmen_distance_ackerman_traj((carmen_robot_and_trailers_traj_point_t *)&moving_object[i].pose, (carmen_robot_and_trailers_traj_point_t *)&moving_object[i + 1].pose);
...
Detector::detect(carmen_obstacle_distance_mapper_map_message *current_map,
				 ...
				 int goal_index,
				 int rddf_pose_index,
				 carmen_robot_and_trailers_traj_point_t robot_pose_,
				 double circle_radius,
				 double displacement,
				 double timestamp)
{
	carmen_robot_and_trailers_traj_point_t robot_pose = {robot_pose_.x, robot_pose_.y, robot_pose_.theta, 0.0, robot_pose_.v, robot_pose_.phi};
...
//	printf("w %d, i %d\n", obstacle_already_detected, rddf_pose_index);
	...
	carmen_robot_and_trailers_pose_t front_car_pose_ = carmen_collision_detection_displace_car_pose_according_to_car_orientation(&rddf->poses[rddf_pose_index], disp);
...
double
Detector::get_moving_obstacle_distance(carmen_robot_and_trailers_traj_point_t robot_pose)
{

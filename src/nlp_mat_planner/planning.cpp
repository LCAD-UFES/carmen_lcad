...
std::vector<std::vector<double> > Planning::expandObstacles(double _dist)
{
	...
	// initialize map
	for (int y = 0; y < (int) tmpMap[0].size(); y++)
		for (int x = 0; x < (int) tmpMap.size(); x++)
		{
			tmpMap[x][y] = 2555;
			this->p[x][y].x = -1;
			this->p[x][y].y = -1;
	...
	for (int y = tmpMap[0].size() - 1; y >= 0; y--)
		for (int x = (int) tmpMap.size() - 1; x >= 0; x--)
		{
			if (map[x][y] <= 0 || tmpMap[x][y] == 0)
			{
				if (x + 1 < (int) tmpMap.size())
				...
				if (x - 1 >= 0 && y + 1 < (int) tmpMap[0].size())
					if (tmpMap[x - 1][y + 1] + d2 < tmpMap[x][y])
					{
						this->p[x][y] = this->p[x - 1][y + 1];
						tmpMap[x][y] = sqrt((x - this->p[x][y].x) * (x - this->p[x][y].x) + (y - this->p[x][y].y) * (y - this->p[x][y].y));
					}
				if (y + 1 < (int) tmpMap[0].size())
					if (tmpMap[x][y + 1] + d1 < tmpMap[x][y])
					{
						this->p[x][y] = this->p[x][y + 1];
						tmpMap[x][y] = sqrt((x - this->p[x][y].x) * (x - this->p[x][y].x) + (y - this->p[x][y].y) * (y - this->p[x][y].y));
					}
				if (x + 1 < (int) tmpMap.size() && y + 1 < (int) tmpMap[0].size())
					if (tmpMap[x + 1][y + 1] + d2 < tmpMap[x][y])
					{
						this->p[x][y] = this->p[x + 1][y + 1];
						tmpMap[x][y] = sqrt((x - this->p[x][y].x) * (x - this->p[x][y].x) + (y - this->p[x][y].y) * (y - this->p[x][y].y));
...
std::vector<std::vector<double> > Planning::pathDR(int _x, int _y)
{
	...
	for (int x = 0; x < (int) p2.size(); x++)
		this->p2[x].resize(map[0].size());

	// initialize map
	for (int y = 0; y < (int) this->path[0].size(); y++)
		for (int x = 0; x < (int) this->path.size(); x++)
		{
...
// 	std::cout<<"Inicia " <<std::endl;
	...
	while (count != 0)
	{
		count = 0;
		//perform the first pass
		for (int y = 1; y < (int) this->path[0].size() - 1; y++)
		{
			for (int x = 1; x < (int) this->path.size() - 1; x++)
			{

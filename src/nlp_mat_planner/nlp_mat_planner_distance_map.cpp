...
#define OFFROAD_PLANNER_DISTANCE_MAP_HUGE_DISTANCE (1050 * M_SQRT2) // tem que ser maior que o tamanho da diagonal do mapa
...
initialize_distance_measurements(int x_size, int y_size, int goal_x, int goal_y, double *utility, double **distance)
{
	...
	std::fill_n(utility, total_size, OFFROAD_PLANNER_DISTANCE_MAP_HUGE_DISTANCE);
...
update_distance_map_new(double *distance_map_complete, double *cost_map_complete, int x_size, int y_size, int goal_x, int goal_y)
{
	...
	for (int y = 0; y < y_size; y++)
		...
		{
			distance_map[x][y] = OFFROAD_PLANNER_DISTANCE_MAP_HUGE_DISTANCE;
			p2[x][y].x = -1;
			p2[x][y].y = -1;
			p2[x][y].dist = OFFROAD_PLANNER_DISTANCE_MAP_HUGE_DISTANCE;

...
function opts.parse(arg)
   ...
   --save                  (default /dados/neural_mapper_png_dataset/trained_model/)     save trained model here

   Dataset Related:
   --channels              (default 5)
   --datapath              (default /dados/neural_mapper_png_dataset/60mts)
                           dataset location
   --velodyneRadius		   (default 60)
   --dataset               (default cv)          dataset type: cv(CamVid)/cs(cityscapes)/su(SUN)
   --cachepath             (default /dados/neural_mapper_png_dataset/torch_saved_dataset)     cache directory to save the loaded dataset

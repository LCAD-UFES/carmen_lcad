...
void
publish_point_cloud(cv::Mat imgdepth, int number_of_rows, int number_of_cols, double timestamp, unsigned char *gray)
{
	carmen_velodyne_variable_scan_message msg;

	msg.host = carmen_get_host();
	msg.timestamp = timestamp;
	msg.number_of_shots = number_of_cols; // 640

	//alloc
	msg.partial_scan = (carmen_velodyne_shot *) malloc(msg.number_of_shots * sizeof(carmen_velodyne_shot));
	
	double angle = -25.0; // horizontal_angle
	...
	// }
	// double angle = +15.0;
	// double delta_angle = 32.25 / (double) number_of_cols;
	// for (int i = 0; i < number_of_cols; i++){
	// 	angle -= delta_angle;
	// 	printf("%.3f ", angle);
	// }
	...
	for (int i = 0; i < number_of_cols; i++)
	{
		...
		for (int j = 0; j < msg.partial_scan[i].shot_size; j++)
		{
			...
			msg.partial_scan[i].distance[j] = 4800;
...
{
	Mat open_cv_image = Mat(stereo_image->height, stereo_image->width, CV_8UC3, stereo_image->raw_right, 0); 
	//cv::Rect myROI(0, 190, stereo_image->width, stereo_image->height - 190);
	//open_cv_image = open_cv_image(myROI);
	unsigned char * depth_pred = libdpt_process_image(stereo_image->width, stereo_image->height, stereo_image->raw_right, stereo_image->timestamp);
	...
	publish_point_cloud(imgdepth, imgdepth.rows, imgdepth.cols, stereo_image->timestamp, image_gray);

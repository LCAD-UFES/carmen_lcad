...
class LowStateRPYNode : public rclcpp::Node
{
public:
    LowStateRPYNode() : Node("lowstate_rpy_node")
    {
        sub_ = this->create_subscription<unitree_go::msg::LowState>(
            "/lf/lowstate", 10,
            std::bind(&LowStateRPYNode::callback, this, std::placeholders::_1));
...
private:
    ...
    {
		double return_value;
        double roll = msg->imu_state.rpy[0]; // Roll
        double pitch = msg->imu_state.rpy[1]; // Pitch
        double yaw = msg->imu_state.rpy[2]; // Yaw
		carmen_param_set_double("lidar10_roll", roll, &return_value);
		carmen_param_set_double("lidar10_pitch", pitch, &return_value);
		carmen_param_set_double("lidar11_roll", roll, &return_value);
		carmen_param_set_double("lidar11_pitch", pitch, &return_value);
    }
...
//    std::vector<LidarScan> scans{N_SCANS, LidarScan{w, h, info.format.udp_profile_lidar}};
	...
	auto node = std::make_shared<LowStateRPYNode>();

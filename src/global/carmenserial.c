...
int carmen_serial_connect(int *dev_fd, char *dev_name)
{
  ...
  *dev_fd = open(dev_name, O_RDWR | O_SYNC | O_NOCTTY | O_NONBLOCK, 0);
...
int carmen_serial_read_cts(int fd)
{
	int cts; //serial CTS (Clear To Send)
	int bits;

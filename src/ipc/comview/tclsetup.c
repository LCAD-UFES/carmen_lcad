 ...
 *
 * $Source: /afs/cs.cmu.edu/project/TCA/Master/tcaV8/tools/comview/tclsetup.c,v $ 
 * REVISION HISTORY
 * $Revision: 1.1 $
 * $Date: 1997/05/29 22:06:16 $
 * $Author: reids $
 *
 * $Log: tclsetup.c,v $
 * Revision 1.1  1997/05/29 22:06:16  reids
 * Adding a few more files that are needed.
 *
 * Revision 1.15  97/05/01  17:39:39  whelan
 * Modified tclsetup.c to check for latest Tk lib, not Tcl lib.
 * 
...
int init_grafx(char *filename, int file_selected, char *set_filename,
	       char *zoom, int zoom_found, char *comview_directory,
	       int tcl_prompt)
{
  char **fake_argv;
  char *startup = "comview";
  ...
  sprintf(startup_string,"source %s/%s", comview_directory, STARTUP_FILE);

  if (tcl_prompt) {
    /* For debugging purposes this will result in a wish prompt */
    Tk_Main(0, fake_argv, (Tcl_AppInitProc*) Tcl_AppInit);
  } else {
...
int Tcl_AppInit(Tcl_Interp *interp)
{
  ...
  My_Tcl_Eval(interp, load_filename);
  My_Tcl_Eval(interp, stngs_filename);
  My_Tcl_Eval(interp, file_sel_string);
  My_Tcl_Eval(interp, zoom_init_str);
  My_Tcl_Eval(interp, set_dir_string);

  sprintf(tcl_command, "set GEOMETRY_SIZE %d", screen_size);
  My_Tcl_Eval(interp, tcl_command);

  My_Tcl_Eval(interp, startup_string);

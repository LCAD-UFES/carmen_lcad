...
//TrajectoryLookupTable::TrajectoryDimensions
//compute_trajectory_dimensions(TrajectoryLookupTable::TrajectoryControlParameters &tcp, int i_v0,
//		vector<carmen_robot_and_trailer_path_point_t> &path)
//{
//	double d_i_phi = get_i_phi_by_index(tcp.k[0]);
//	double d_i_v0 = get_initial_velocity_by_index(i_v0);
//	TrajectoryLookupTable::TrajectoryDimensions td;
//	path = simulate_car_from_parameters(td, tcp, d_i_v0, 0.0); // beta = 0.0
//
//	return (td);
//}
//
...
//						vector<carmen_robot_and_trailer_path_point_t> path;
//						TrajectoryLookupTable::TrajectoryDimensions td = compute_trajectory_dimensions(tcp, i_v, path, false);
//						//                        td = compute_trajectory_dimensions(tcp, i_v, path, true);
//						//                        FILE *gnuplot_pipe = popen("gnuplot -persist", "w");
...
//									//	                                compute_trajectory_dimensions(trajectory_lookup_table[tdd.dist][tdd.theta][tdd.d_yaw][tdd.phi_i][tdd.v_i], i_v, path, true);
//									//                                    FILE *gnuplot_pipe = popen("gnuplot -persist", "w");
//									//                                    fprintf(gnuplot_pipe, "plot './gnuplot_path.txt' using 1:2:3:4 w vec size  0.3, 10 filled\n");
//									//                                    fflush(gnuplot_pipe);
//									//                                    pclose(gnuplot_pipe);

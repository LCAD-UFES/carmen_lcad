...
static int
found_possible_parking_space(carmen_point_t car_pose, carmen_map_p map, carmen_robot_ackerman_config_t *car_config)
{
...
static void
found_parking_space(carmen_localize_ackerman_globalpos_message car)
{
...
static int
step_point_reached(carmen_point_t p, carmen_point_t step_point)
{
	...
	}else //if goal is set
	{
		...
		switch (step) {
			...
			case 5:
				...
				break;
			case 6:
				if (car.v < 0.001) {// && DIST2D(car.globalpos,path[2]) < 0.8 && fabs(car.globalpos.theta-path[2].theta) < 0.0872664626) {
...
static int read_parameters(int argc, char **argv)
{
    ...
    carmen_param_t param_list[] =
    {
            ...
            {(char*)"robot",  (char*)"max_steering_angle", CARMEN_PARAM_DOUBLE, &(robot_max_steering_angle), 0, NULL},

...
{
	int i;
	double y, inc_vertical;
	double *vertical_correction;
	int height = roi_end - roi_ini;

	inc_vertical = 0.5 + ((double) height / (double) vertical_resolution);

	vertical_correction = (double *) calloc(vertical_resolution, sizeof(double));
	carmen_test_alloc(vertical_correction);

	for (y = roi_ini, i = vertical_resolution/interface.stereo_stride_y - 1; y < roi_end; y += inc_vertical*interface.stereo_stride_y , i--)
...
{
	int j;
	double x, inc_horizontal;
	double *horizontal_correction;

	int width = roi_end - roi_ini;

	inc_horizontal = 0.5 + ((double) width / (double) horizontal_resolution);

	horizontal_correction = (double *) calloc(horizontal_resolution, sizeof(double));
	carmen_test_alloc(horizontal_correction);

	for (x = roi_ini, j = horizontal_resolution/interface.stereo_stride_x - 1; x <  roi_end; x += inc_horizontal*interface.stereo_stride_x, j--)
		...
		double range_max, int vertical_roi_ini, int vertical_roi_end, int horizontal_roi_ini, int horizontal_roi_end, unsigned char *image)
{
	...
	for (x = horizontal_roi_ini, j = horizontal_resolution/interface.stereo_stride_x - 1; x <  horizontal_roi_end; x += interface.stereo_stride_x*inc_horizontal, j--)
	{
		...
		for (y = vertical_roi_ini, i = vertical_resolution/interface.stereo_stride_y - 1; y < vertical_roi_end; y += interface.stereo_stride_y*inc_vertical, i--)
		{
			...
			p.x = x;
			p.y = y;
			point = reproject_single_point_to_3D(&interface, p, disparity[(int)(y * (double)interface.width + x)]);
		...
		unsigned short range_max, int vertical_roi_ini, int vertical_roi_end, int horizontal_roi_ini, int horizontal_roi_end, unsigned char *image)
{
	...
	for (x = horizontal_roi_ini, j = horizontal_resolution/interface.stereo_stride_x - 1; x <  horizontal_roi_end; x += interface.stereo_stride_x*inc_horizontal, j--)
	{
		...
		for (y = vertical_roi_ini, i = vertical_resolution/interface.stereo_stride_y - 1; y < vertical_roi_end; y += interface.stereo_stride_y*inc_vertical, i--)
		{
			...
			p.x = x;
			p.y = y;
			point = reproject_single_point_to_3D_with_depth(interface, p, depth[(int)(y * (double)interface.width + x)]);

...
pedestrian_about_to_enter_crosswalk(t_point_cloud_struct moving_object, carmen_annotation_t pedestrian_track_annotation, double ray)
{
	...
	printf("OR%lf OOR%lf DOR%f\n", orientantion, moving_object.orientation,abs(carmen_normalize_theta(orientantion - moving_object.orientation)));
	double pedestrian_cross_distance = DIST2D(desplaced_crosswalk_pose, moving_object.object_pose);
	...
	//if (pedestrian_cross_distance*tan(carmen_normalize_theta(orientantion - moving_object.orientation)) < ray)
	{
		return (false);
	}

	if (moving_object.linear_velocity > 0.2)
		pedestrian_time_to_crosswalk_border = (pedestrian_cross_distance - ray) / moving_object.linear_velocity;
	...
	for (int i = 0; i < moving_objects->num_point_clouds; i++)
	{
		if (strcmp(moving_objects->point_clouds[i].model_features.model_name, "pedestrian") == 0)
		{
			if ((DIST2D(moving_objects->point_clouds[i].object_pose, desplaced_crosswalk_pose) < ray &&                    // Inside the crosswalk circle
				 moving_objects_msg->point_clouds[i].linear_velocity > 0.3 &&                                              // Moving faster than 0.3m/s
				 abs(carmen_normalize_theta(current_globalpos_msg->globalpos.theta - moving_objects_msg->point_clouds[i].orientation)) > 0.2) &&     // Not moving parallel to the car (sideways with the crosswalk)
			     abs(carmen_normalize_theta(current_globalpos_msg->globalpos.theta + M_PI - moving_objects_msg->point_clouds[i].orientation)) > 0.2)     // Not moving parallel to the car  on oposite direction(sideways with the crosswalk))//||
...
pedestrian_track_busy_new(carmen_moving_objects_point_clouds_message *moving_objects_msg, carmen_annotation_t pedestrian_track_annotation)
{
	...
	switch (crosswalk_state)
	{
		...
		case Stopping_Busy_Crosswalk:
			...
			else if (current_globalpos_msg->v < 0.15 && DIST2D(current_globalpos_msg->globalpos, desplaced_crosswalk_pose) < 15.0) // || dist stop point < 2.0
			{
...
double
nearest_distance_to_rddf(carmen_ackerman_traj_point_t *carmen_rddf_poses_ahead, carmen_ackerman_traj_point_t *carmen_rddf_poses_back,
		int carmen_rddf_num_poses_ahead, int carmen_rddf_num_poses_back, carmen_vector_3D_t object, int* nearest_rddf_point_index)
{
	if (nearest_rddf_point_index != NULL)
		*nearest_rddf_point_index = -1;

	if (carmen_rddf_poses_ahead == NULL || carmen_rddf_num_poses_ahead == 0)
		return  -1;

	double nearest_distance = 1000;
	for (int i = 0; i < carmen_rddf_num_poses_back; i++)
	{
		double curr_dist = DIST2D_P(&carmen_rddf_poses_back[i], &object);
		if (curr_dist < nearest_distance)
		{
			nearest_distance = curr_dist;
			if (nearest_rddf_point_index != NULL)
				*nearest_rddf_point_index = carmen_rddf_num_poses_back-i-1;
		}
	}
	for (int i = 0; i < carmen_rddf_num_poses_ahead; i++)
	{
		double curr_dist = DIST2D_P(&carmen_rddf_poses_ahead[i], &object);
		if (curr_dist < nearest_distance)
		{
			nearest_distance = curr_dist;
			if (nearest_rddf_point_index != NULL)
				*nearest_rddf_point_index = carmen_rddf_num_poses_back+i;
		}
	}
...
void
displace_rddf_point (carmen_ackerman_traj_point_t* original_point, carmen_ackerman_traj_point_t* displaced_point, double displacement)
{
	double displace_direction = carmen_normalize_theta(original_point->theta + M_PI/2.0);
	printf("POINT = %lf , %lf\n", displaced_point->x, displaced_point->y);
	displaced_point->x = original_point->x + displacement * cos(displace_direction);
	displaced_point->y = original_point->y + displacement * cos(displace_direction);
	displaced_point->theta = original_point->theta;
	displaced_point->phi   = original_point->phi;
	printf("DISPLACED = %lf , %lf\n", displaced_point->x, displaced_point->y);
	displaced_point->v = 4.0;
}


#define AVOIDER_DIST 20
int
pedestrian_avoider(carmen_ackerman_traj_point_t *carmen_rddf_poses_ahead,carmen_ackerman_traj_point_t *carmen_rddf_poses_back, 
		int carmen_rddf_num_poses_ahead, int carmen_rddf_num_poses_back, carmen_moving_objects_point_clouds_message *moving_objects)
{
	...
	for (int i = 0; i < moving_objects->num_point_clouds; i++)
	{
		...
		double nearest_distance = nearest_distance_to_rddf(carmen_rddf_poses_ahead,carmen_rddf_poses_back, carmen_rddf_num_poses_ahead,
				carmen_rddf_num_poses_back, moving_objects->point_clouds[i].object_pose, &nearest_rddf_point_index);
		
		
		printf("nearest Distance = %lf\n", nearest_distance);
		
		if (nearest_distance < 0.0 || nearest_distance > 2.0 || nearest_rddf_point_index < 0 ||
			nearest_rddf_point_index > carmen_rddf_num_poses_back+carmen_rddf_num_poses_ahead)
			break;

		double rddf_max_displacement = 2.0 - nearest_distance;
		double rddf_displacement_increment = rddf_max_displacement / (double)AVOIDER_DIST;

		if (nearest_rddf_point_index < carmen_rddf_num_poses_back)
			displace_rddf_point(&carmen_rddf_poses_back[carmen_rddf_num_poses_back - nearest_rddf_point_index - 1], 
								&carmen_rddf_poses_back[carmen_rddf_num_poses_back - nearest_rddf_point_index - 1], rddf_max_displacement);
		else if (nearest_rddf_point_index < carmen_rddf_num_poses_back+carmen_rddf_num_poses_ahead)
			displace_rddf_point(&carmen_rddf_poses_ahead[nearest_rddf_point_index-carmen_rddf_num_poses_back],
								&carmen_rddf_poses_ahead[nearest_rddf_point_index-carmen_rddf_num_poses_back], rddf_max_displacement);

// 		carmen_annotation_t annotation;
// 		char annotation_description[1024];
// 		annotation.annotation_description = (char *) calloc (1024, sizeof(char));
// 		strcpy(annotation.annotation_description, "RDDF_ANNOTATION_TYPE_SPEED_LIMIT");
// //		
// 		annotation.annotation_type = ;
// 		annotation.annotation_code = ;
// 		annotation.annotation_point.x = ;
// 		annotation.annotation_point.y = ;
// 		anottation.annotation_point.z = 0.0;
...
// 		annotation_read_from_file.push_back(annotation);
		...
		for (int j=0; j<AVOIDER_DIST ; j++)
		{
			printf("%d\n",nearest_rddf_point_index-AVOIDER_DIST+j);
			int back_rddf_index = nearest_rddf_point_index-AVOIDER_DIST+j;
			int ahead_rddf_index = nearest_rddf_point_index+AVOIDER_DIST-j;
			if (back_rddf_index >= 0)
			{
				if (back_rddf_index < carmen_rddf_num_poses_back)
				{
					back_rddf_index = carmen_rddf_num_poses_back - back_rddf_index - 1;	
					displace_rddf_point(&carmen_rddf_poses_back[back_rddf_index] ,&carmen_rddf_poses_back[back_rddf_index],rddf_displacement_increment*j);
				}
				else
				{
					back_rddf_index -= carmen_rddf_num_poses_back;
					displace_rddf_point(&carmen_rddf_poses_ahead[back_rddf_index] ,&carmen_rddf_poses_ahead[back_rddf_index],rddf_displacement_increment*j);
				}
			}
			if (ahead_rddf_index < carmen_rddf_num_poses_back+carmen_rddf_num_poses_ahead)
			{
				if (ahead_rddf_index < carmen_rddf_num_poses_back)
				{
					ahead_rddf_index = carmen_rddf_num_poses_back - ahead_rddf_index - 1;
					displace_rddf_point(&carmen_rddf_poses_back[ahead_rddf_index] ,&carmen_rddf_poses_back[ahead_rddf_index],rddf_displacement_increment*j);
				}
				else
				{
					ahead_rddf_index -= carmen_rddf_num_poses_back;
					displace_rddf_point(&carmen_rddf_poses_ahead[ahead_rddf_index] ,&carmen_rddf_poses_ahead[ahead_rddf_index],rddf_displacement_increment*j);
				}
			}
		}
	}
...
init_python()
{
	...
	if (python_frenet_test == NULL || !PyCallable_Check(python_frenet_test))
	{
		Py_DECREF(python_module);
		Py_Finalize();
		exit (printf("Error: Could not load the python_frenet_test function.\n"));
	}
	printf("functions_initialized\n");
	return 0;
}
		...
		int carmen_rddf_num_poses_ahead, int carmen_rddf_num_poses_back __attribute__ ((unused)))
{
	...
	// for (int i=carmen_rddf_num_poses_back-1; i>=0; i--)
	// {
...
PyListObject *
get_pedestrians(carmen_moving_objects_point_clouds_message *moving_objects)
{
	PyObject *list = Py_BuildValue("[]");
	if (moving_objects == NULL)
		return (PyListObject *)list;	

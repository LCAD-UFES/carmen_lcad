...
carmen_pose_3D_t get_world_pose(carmen_pose_3D_t local_pose) {
	...
	carmen_pose_3D_t world_pose;
	carmen_pose_3D_t parent_world_pose = get_world_pose(pose->parent);
	
	world_pose.position.x = parent_world_pose.position.x + local_pose.position.x;
	world_pose.position.y = parent_world_pose.position.y + local_pose.position.y;
	world_pose.position.z = parent_world_pose.position.z + local_pose.position.z;
	
	parent_world_rotation = create_rotation_matrix(parent_world_pose.orientation);
	local_rotation = create_rotation_matrix(local_pose.orientation);
	
	world_rotation = multiply_matrix_matrix(parent_world_rotation, world_rotation);
	
	destroy_rotation_matrix(parent_world_rotation);
	destroy_rotation_matrix(local_rotation);
	
	
	world_pose.orientation = get_angles_from_rotation_matrix(world_rotation);

...
public: 
    ...
    // Reconstroi um descritor+chaves já prontos (ex: lido de um .scd salvo em
    // disco), sem precisar do point cloud original. Usado no boot do modo de
    // localização pra repopular polarcontexts_/polarcontext_invkeys_mat_.
    void loadScancontextAndKeys( const Eigen::MatrixXd & _sc );
...
public:
    ...
    // 0.3 -> 0.5. Com 0.3 NENHUM candidato passava: num trajeto real as distancias
    // de Scan Context ficaram todas entre 0.388 e 0.72 ("[Not loop] Nearest distance"),
    // entao o robo passava pelo mesmo ponto e o loop nunca fechava por contexto.
    // A faixa 0.4-0.6 e' a indicada logo abaixo justamente para o caso deste projeto:
    // performSCLoopClosure() usa kernel robusto (Cauchy) E checa historyKeyframeFitnessScore
    // no ICP, que sao as duas protecoes contra falso-positivo que permitem afrouxar aqui.

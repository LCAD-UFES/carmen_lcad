...
def process_svg_file(svg_file):
    global args, total_files, count_files
    count_files += 1
...
def read_parameters():
    ...
    parser.add_argument('-o', '--outputdir', help='RDDF output directory   (default: .)', type=_dir, default='.')
    ...
    if args.dist_points <= 0.0:
        usage_exit('RDDF distance between waypoints must be a positive value in meters   (default: 0.5): {}\n\n'.format(args.dist_points))
    
    
def main():
    global parser, args, total_files, count_files
    print
    
    read_parameters()
    
    filelist = get_filelist(args.filelist)
    total_files = len(args.filename) + len(filelist)
    count_files = 0

    for (f_list, f_list_from) in ((args.filename, '[commandline]'), (filelist, args.filelist)):
        if f_list:
            print("********** Processing {} SVG file{} from '{}'".format(len(f_list), 's' * (len(f_list) > 1), f_list_from)) 
            for f in f_list:
                process_svg_file(f)

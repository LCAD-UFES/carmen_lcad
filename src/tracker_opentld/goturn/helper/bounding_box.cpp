...
double BoundingBox::compute_output_width() const {
  ...
  return STD_MAX(1.0, output_width);
}
...
double BoundingBox::compute_output_height() const {
  ...
  return STD_MAX(1.0, output_height);
}
...
void BoundingBox::Uncenter(const cv::Mat& raw_image,
  ...
  bbox_uncentered->x1_ = STD_MAX(0.0, x1_ + search_location.x1_ - edge_spacing_x);
  bbox_uncentered->y1_ = STD_MAX(0.0, y1_ + search_location.y1_ - edge_spacing_y);
  bbox_uncentered->x2_ = STD_MIN(static_cast<double>(raw_image.cols), x2_ + search_location.x1_ - edge_spacing_x);
  bbox_uncentered->y2_ = STD_MIN(static_cast<double>(raw_image.rows), y2_ + search_location.y1_ - edge_spacing_y);
}
...
double BoundingBox::edge_spacing_x() const {
  ...
  return STD_MAX(0.0, output_width / 2 - bbox_center_x);
}
...
double BoundingBox::edge_spacing_y() const {
  ...
  // being the amount that the output "sticks out" beyond the edge of the image.
  return STD_MAX(0.0, output_height / 2 - bbox_center_y);
}
...
void BoundingBox::Shift(const cv::Mat& image,
  ...
  while ((new_width < 0 || new_width > image.cols - 1) && num_tries_width < kMaxNumTries) {
    ...
    if (shift_motion_model) {
      width_scale_factor = STD_MAX(min_scale, STD_MIN(max_scale, sample_exp_two_sided(lambda_scale_frac)));
    } else {
    ...
    new_width = STD_MAX(1.0, STD_MIN(static_cast<double>(image.cols - 1), new_width));
    num_tries_width++;
  }
  ...
  while ((new_height < 0 || new_height > image.rows - 1) && num_tries_height < kMaxNumTries) {
    ...
    if (shift_motion_model) {
      height_scale_factor = STD_MAX(min_scale, STD_MIN(max_scale, sample_exp_two_sided(lambda_scale_frac)));
    } else {
      const double rand_num = sample_rand_uniform();
    ...
    new_height = STD_MAX(1.0, STD_MIN(static_cast<double>(image.rows - 1), new_height));
    num_tries_height++;
  }
  ...
  while ((first_time_x ||
    ...
    new_center_x = STD_MIN(image.cols - new_width / 2, STD_MAX(new_width / 2, new_x_temp));
    first_time_x = false;
    num_tries_x++;
  }
  ...
  while ((first_time_y ||
    ...
    // Make sure that the window stays within the image.
    new_center_y = STD_MIN(image.rows - new_height / 2, STD_MAX(new_height / 2, new_y_temp));
    first_time_y = false;
    num_tries_y++;
  }
...
double BoundingBox::compute_intersection(const BoundingBox& bbox) const {
  const double area = STD_MAX(0.0, STD_MIN(x2_, bbox.x2_) - STD_MAX(x1_, bbox.x1_)) * STD_MAX(0.0, STD_MIN(y2_, bbox.y2_) - STD_MAX(y1_, bbox.y1_));
  return area;
}

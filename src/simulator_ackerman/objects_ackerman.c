...
carmen_robot_and_trailers_traj_point_t *search_old(carmen_robot_and_trailers_traj_point_t key, carmen_robot_and_trailers_traj_point_t *lane, int size)
{
	int begin = 0;
	int end = size - 1;
...
find_nearest_pose_in_lane(carmen_robot_and_trailers_traj_point_t pose, carmen_robot_and_trailers_traj_point_t *lane, int size, int *index_in_lane)
{
		...
		if (nearby_lane_merges[merge].index_of_node_in_current_lane == *node_index_in_lane)
		{
			*another_lane_id = nearby_lane_merges[merge].target_lane_id;
			for (int another_lane_index = 0; another_lane_index < road_network_message->number_of_nearby_lanes; another_lane_index++)
			{
				...
				{
					carmen_robot_and_trailers_traj_point_t *another_lane = &(road_network_message->nearby_lanes[road_network_message->nearby_lanes_indexes[another_lane_index]]);
...
carmen_robot_and_trailers_traj_point_t *
find_nearest_pose_in_the_nearest_lane(carmen_object_ackerman_t *object, carmen_robot_and_trailers_traj_point_t pose,
		int *lane_size, int *index_in_lane)
{
	carmen_robot_and_trailers_traj_point_t *nearest_pose_in_the_nearest_lane = NULL;
	...
	if (object->lane_id == 0)
	{
		...
		for (int i = 0; i < road_network_message->number_of_nearby_lanes; i++)
		{
			...
			int index;
			carmen_robot_and_trailers_traj_point_t *nearest_pose_in_the_lane = find_nearest_pose_in_lane(pose, lane, size, &index);
		...
		for (int i = 0; i < road_network_message->number_of_nearby_lanes; i++)
		{
			if (object->lane_id == road_network_message->nearby_lanes_ids[i])
			{
				...
				*lane_size = road_network_message->nearby_lanes_sizes[i];
				...
				{
					int another_lane_id;
					carmen_robot_and_trailers_traj_point_t *pose_in_another_lane;
...
//	printf(" v %lf, dt %lf, x %lf, y %lf\n", new_object.tv, dt, new_object.x1, new_object.y1);
	...
	int lane_size;
	int index_in_lane;
	carmen_robot_and_trailers_traj_point_t *pose_in_lane = find_nearest_pose_in_the_nearest_lane(&new_object, pose_ahead, &lane_size, &index_in_lane);
	if (!pose_in_lane)
		return;

	int status;
	carmen_robot_and_trailers_traj_point_t next_pose;
	if (index_in_lane == 0)

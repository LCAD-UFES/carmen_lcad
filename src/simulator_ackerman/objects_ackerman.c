...
check_list_capacity(void)
{
  if (traj_object_list == NULL) 
	...
	calloc(10, sizeof(carmen_traj_point_t));
      ...
      object_list = (carmen_object_ackerman_t *)
	calloc(10, sizeof(carmen_object_ackerman_t));
	...
	realloc(traj_object_list, sizeof(carmen_traj_point_t)*list_capacity);
      ...
      object_list = (carmen_object_ackerman_t *)
	realloc(object_list, sizeof(carmen_object_ackerman_t)*list_capacity);
      carmen_test_alloc(object_list);
    }
}

void
carmen_simulator_ackerman_create_object(double x, double y, double theta,
...
void 
carmen_simulator_ackerman_add_objects_to_laser(carmen_laser_laser_message * laser, 
				      carmen_simulator_ackerman_config_t 
...
void 
carmen_simulator_ackerman_get_object_poses(int * num, 
...
void
carmen_simulator_ackerman_initialize_object_model(int argc, char *argv[])
{
...
find_object(IPC_CONTEXT_PTR context)
{
  ...
  for (i = 0; i < num_objects; i++) 
    {
      if (object_list[i].type == CARMEN_SIMULATOR_ACKERMAN_OTHER_ROBOT &&
...
static void
globalpos_handler(carmen_localize_ackerman_globalpos_message *msg)
{
...
void 
carmen_simulator_ackerman_add_robot(char *program_name, char *robot_central)
{
  char buffer[1024];
...
static void
update_other_robot(carmen_object_ackerman_t *object)
{
  IPC_CONTEXT_PTR current_context;

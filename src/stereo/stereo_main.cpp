...
void compute_depth_map(carmen_bumblebee_basic_stereoimage_message *stereo_image) {
...
*/
  left_roi_image = stereo_image->raw_left + vertical_ROI_ini * 3
  * bumblebee_basic_width;
  right_roi_image = stereo_image->raw_right + vertical_ROI_ini * 3
  * bumblebee_basic_width;
  disparity_roi_map = disparity_message.disparity + vertical_ROI_ini
  * bumblebee_basic_width;
  ...
  case 'E':
    ...
    for (i = 0, y = vertical_ROI_ini; y < vertical_ROI_end; y++, i++)
      for (j = 0, x = horizontal_ROI_ini; x < horizontal_ROI_end; x++, j++)
        disparity_message.disparity[y * bumblebee_basic_width + x] = MAX(right_disparity[i * ROI_width + j], 0.0);
...
void stereo_algorithm_initialization() {
  disparity_message.disparity_size = bumblebee_basic_height
  * bumblebee_basic_width;
  disparity_message.disparity = (float *) calloc(
      (disparity_message.disparity_size), sizeof(float));
  ...
  if (stereo_scalew > bumblebee_basic_width && stereo_scaleh > bumblebee_basic_height) {
    ...
    image_left_copy = cvCreateImage(cvSize(bumblebee_basic_width,
        bumblebee_basic_height), IPL_DEPTH_8U, 3);
    image_right_copy = cvCreateImage(cvSize(bumblebee_basic_width,
        bumblebee_basic_height), IPL_DEPTH_8U, 3);
    image_left_rescale = cvCreateImage(
        cvSize(stereo_scalew, stereo_scaleh), IPL_DEPTH_8U, 3);
    image_right_rescale = cvCreateImage(
        cvSize(stereo_scalew, stereo_scaleh), IPL_DEPTH_8U, 3);
    raw_image_left_rescale = (unsigned char *) calloc((stereo_scalew
        * stereo_scaleh) * 3, sizeof(unsigned char));
    raw_image_right_rescale = (unsigned char *) calloc((stereo_scalew
        * stereo_scaleh) * 3, sizeof(unsigned char));
...
void image_handler(carmen_bumblebee_basic_stereoimage_message *stereo_image) {
  ...
  //  memset(stereo_image->raw_right,0,sizeof(unsigned char) * 3 * vertical_ROI_ini * bumblebee_basic_width);
  //  memset(stereo_image->raw_right +  3 * vertical_ROI_end * bumblebee_basic_width,0,sizeof(unsigned char) * 3 * (bumblebee_basic_height - vertical_ROI_end) * bumblebee_basic_width);
...
void subscribe_bumblebee_basic_messages(int camera_organization)
{
  carmen_bumblebee_basic_subscribe_stereoimage(camera_organization, NULL,
      (carmen_handler_t) image_handler, CARMEN_SUBSCRIBE_LATEST);
}

...
//				sensor_data->ray_position_in_the_floor[tid][0].y, 1);
		...
		if (create_map_sum_and_count)
			if (height_level == 0)
				...
				carmen_prob_models_update_sum_and_count_of_cells_hit_by_rays(&occupancy_map, &sum_occupancy_map, &count_occupancy_map, sensor_params, sensor_data, highest_sensor, safe_range_above_sensors, tid, safe_height_from_ground);
		else
			if (height_level == 0)
...
mapper_change_map_origin_to_another_map_block(carmen_position_t *map_origin)
{
	...
	if (first_time)
	{
		initialize_first_map_block_origin(&occupancy_map, map_origin, 'm'-height_level);
		...
		carmen_grid_mapping_create_new_map(&new_carmen_map, occupancy_map.config.x_size, occupancy_map.config.y_size, occupancy_map.config.resolution, 'm'-height_level);
		if (height_level == 0)
		{
			if (use_remission)
			{
	...
	if (carmen_grid_mapping_is_map_changed(map_origin, x_origin, y_origin))
	{
		...
		if (update_and_merge_with_mapper_saved_maps)
		{
			...
			{
				carmen_grid_mapping_save_block_map_by_origin(map_path, 'm'-height_level, &occupancy_map);
				if (height_level == 0)
				{
					if (use_remission)
					{
		...
		{
			carmen_grid_mapping_update_map_buffer(&occupancy_map, 'm'-height_level);
...
//	add_moving_objects(&map, &moving_objects_message);
	...
	if (height_level > 0)
		carmen_mapper_publish_map_level_message(&occupancy_map, timestamp, height_level);
	else
...
int
carmen_parse_polygon_file (double **polygon)
{
	FILE *poly;
	int n_points, h_lvl;
	char *poly_file;
...
carmen_mapper_update_cells_bellow_robot(carmen_point_t pose, carmen_map_t *map, double prob)
{
	...
	if (load_polygon)
	{
		col_n_points = carmen_parse_polygon_file(&collision);
		load_polygon = 0;
		printf("Col Loaded\n");
	}
	...
	for (int i=0; i<col_n_points; i++)
	{
		...
		for (int j = -pix_radius; j <= pix_radius; j++)
		{
			...
			{
				if (grid_collision[4*i]+j >= 0 && grid_collision[4*i]+j < map->config.x_size && grid_collision[4*i+1]+k >= 0 && grid_collision[4*i+1]+k < map->config.y_size && j*j+k*k <= pix_radius*pix_radius && collision[4*i+3] <= height_level)
				{
...
mapper_initialize(carmen_map_config_t *main_map_config, carmen_robot_ackerman_config_t main_car_config)
{
	...
	carmen_grid_mapping_create_new_map(&occupancy_map, map_config.x_size, map_config.y_size, map_config.resolution, 'm'-height_level);
